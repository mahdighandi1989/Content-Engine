...
def main():
    ...
    # ── «پایه کامل است» شمرده می‌شود، نه حدس زده ──
    # کشِ سنگین کلیدش به ورودی بسته است و تا ورودی عوض نشود یک بار
    # نوشته می‌شود. پس اگر یک اجرای نیمه‌کاره بنویسدش، همان نیمه تا ابد
    # می‌مانَد و هر اجرا دو ساعت‌ونیم را دوباره می‌سوزاند، بی هیچ خطایی.
    # این تابع دروازهٔ آن نوشتن است، پس خودش باید آزموده شود.
    wk = tempfile.mkdtemp()
    rt = tempfile.mkdtemp()
    ...
    eq(VT.baseReady_(rt, wk)["ok"], False, "بی mute، پایه کامل نیست")
