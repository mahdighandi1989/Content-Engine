...
def main():
    ...
    # ── دانلودِ درایو: تلاشِ دوباره، و کم‌آمدن که خطاست ──
    # اجرای ۴۶ روی ضبطِ **هفتم از هشت** افتاد: «have had many accesses»
    # — سهمیهٔ درایو، نه خرابیِ لینک. `gdown` استثنا پرتاب می‌کند، و
    # حلقهٔ آن‌موقع برای «None برگردانَد» نوشته شده بود، پس تحملش هرگز
    # اجرا نشد.
    class _G(object):
        def __init__(self, okAt):
            self.okAt, self.k = okAt, 0

        def download(self, id=None, output=None, quiet=True):
            self.k += 1
            if self.k < self.okAt:
                raise RuntimeError("quota")
            io.open(output, "w").write(u"x" * 200000)

    dtmp = tempfile.mkdtemp()
    eq(VT.gdl_(_G(3), "fid", os.path.join(dtmp, "a"), base=0), True,
       "خطای گذرا با تلاشِ دوباره جبران می‌شود")
    eq(VT.gdl_(_G(99), "fid", os.path.join(dtmp, "b"), base=0), False,
       "و خطای ماندگار در نهایت «نیامد» می‌دهد، نه استثنا")

    # ══ و کم آمدنِ یک ضبط باید **بایستاند** ══
    # نسخهٔ پیشین فقط وقتی می‌ایستاد که هیچ ضبطی نیاید. آن تحمل خطرناک
    # است: `dsSig_` امضا را از فهرستِ شناسه‌ها می‌سازد نه از آنچه دانلود
    # شد، پس دیتاستِ هفت‌تایی با امضای هشت‌تایی در کشِ یک‌بارنوشتِ پایه
    # می‌نشست و تا ابد می‌مانْد.
    vtSrc = io.open(os.path.join(os.path.dirname(os.path.abspath(
        VT.__file__)), "voicetrain.py"), encoding="utf-8").read()
    eq("len(srcs) != len(DRIVE_IDS)" in vtSrc, True,
       "دیتاستِ ناقص ساخته نمی‌شود — شمارش با فهرستِ شناسه‌ها سنجیده می‌شود")
    eq("if not srcs:" in vtSrc, False,
       "و شرطِ کهنهٔ «هیچ‌کدام نیامد» دیگر نیست")

    # ── دو پرسشِ جدا، چون دو تصمیمِ جدا ──
    # `featsReady_` می‌پرسد «دیتاست را دوباره بسازم؟» و `baseReady_`
    # می‌پرسد «این پایه را برای همیشه در کش بگذارم؟». دومی سخت‌گیرتر است
    # و شاهدش شمارش نیست.
    wk = tempfile.mkdtemp()
    rt = tempfile.mkdtemp()
    ...
    eq(VT.featsReady_(rt), False, "پوشهٔ خالی، استخراج نیست")

    for sub in SUB:
        mk_(rt, "logs", VT.VOICE, sub)
    for i_ in (1, 2, 3):
        put_(rt, "logs", VT.VOICE, "0_gt_wavs", "%d.wav" % i_)
    eq(VT.featsReady_(rt), False, "بی ویژگی هم نیست")

    for i_ in (1, 2):
        for sub in SUB[1:]:
            put_(rt, "logs", VT.VOICE, sub, "%d.npy" % i_)
    eq(VT.featsReady_(rt), False, "ویژگیِ نیمه‌کاره هم نیست — دو از سه")

    # ══ و اینجا بندی که اجرای ۴۳ خریدش ══
    # نسخهٔ اول از **هر سه** پوشه برابری با `0_gt_wavs` می‌خواست. داده‌های
    # واقعی: gt=3879، feat=3879، ولی f0=3878 و nsf=3878 — یکی کم، چون
    # `extract_f0` تکه‌ای را که گامِ صدا در آن پیدا نشود کنار می‌گذارد.
    # آموزش همان اجرا دورِ ۵ را تمام کرد، یعنی پایه سالم بود؛ ولی دروازه
    # «ناقص» گفت، پایه ذخیره نشد، و هر اجرا ~۵۲ دقیقه صرفِ ساختِ دوبارهٔ
    # همان چیز می‌کرد — ۱۸٪ از توانِ هر اجرا، بی هیچ خطایی و برای همیشه.
    put_(rt, "logs", VT.VOICE, "3_feature768", "3.npy")
    eq(VT.featsReady_(rt), True,
       "کمبودِ یکیِ f0 استخراج را ناقص نمی‌کند — همان که اجرای ۴۳ نشان داد")
    for i_ in (1, 2):
        os.remove(os.path.join(rt, "logs", VT.VOICE, "2a_f0", "%d.npy" % i_))
    eq(VT.featsReady_(rt), False, "ولی f0ِ اصلاً-نبوده یعنی پاس اجرا نشده")
    put_(rt, "logs", VT.VOICE, "2a_f0", "1.npy")

    # ── و دروازهٔ کش: شاهدش «آموزش راه افتاد» است ──
    # کشِ پایه یک بار نوشته می‌شود و برای همیشه می‌مانَد، پس نباید به یک
    # بازگوییِ تقریبیِ دروازهٔ آموزش تکیه کند. محکم‌ترین شاهدِ موجود این
    # است که `filelistCheck_` فهرست را سالم دید و آموزش واقعاً دور زد.
    mk_(rt, "assets", "hubert_base")
    put_(rt, "assets", "hubert_base", "pytorch_model.bin")
    mk_(rt, "logs", "mute", "0_gt_wavs")
    eq(VT.baseReady_(rt, False)["ok"], False,
       "اجرایی که آموزش نداده، پایه را برای همیشه ثبت نمی‌کند")
    eq(VT.baseReady_(rt, True)["ok"], True, "و اجرایی که داده، می‌کند")

    # ══ هر دو وزنِ پایه، نه یکی ══
    # `have` در خودِ کد همین جفت را می‌سنجد: بی `logs/mute`، سه قدم بعد
    # فهرستِ آموزش ردیف‌های mute را ندارد و کمبود با نامی دیگر ظاهر
    # می‌شود. یک کشِ یک‌بارنوشتِ بی‌mute یعنی آن خطا برای همیشه.
    os.rmdir(os.path.join(rt, "logs", "mute", "0_gt_wavs"))
    eq(VT.baseReady_(rt, True)["ok"], False, "بی mute، پایه کامل نیست")
    ...
    # ══ دو کش، و قراردادی که اگر بشکند هیچ‌چیز قرمز نمی‌شود ══
    # تا ۱۳ سپتامبر یک کش بود و این‌جا فقط می‌پرسید «کلیدِ ذخیره و
    # بازیابی یکی است؟». حالا دو تاست — سنگینِ بازساختنی با کلیدی بسته
    # به ورودی (یک‌بار نوشته می‌شود) و سبکِ بازساختنی‌نیست با کلیدِ هر
    # اجرا — و هر بندِ زیر یک راهِ شکستنِ بی‌صداست که واقعاً ممکن است.
    steps_ = _yamlStrict_(wf)["jobs"]["train"]["steps"]
    cch = [st for st in steps_ if "cache" in str(st.get("uses", ""))]
    fam = {}
    for st in cch:
        k = str(st["with"]["key"])
        fam.setdefault("base" if "-base-" in k else "ckpt", []).append(st)
    eq(sorted(fam), ["base", "ckpt"], "دو خانوادهٔ کش هست، نه یکی")

    for nm in ("base", "ckpt"):
        got = fam[nm]
        eq(len(got), 2, "%s یک برداشتن دارد و یک گذاشتن (%d)" % (nm, len(got)))
        eq(len({str(x["with"]["key"]) for x in got}), 1,
           "کلیدِ ذخیره و بازیابیِ %s یکی است" % nm)
        # ══ مسیرها هم باید مو‌به‌مو یکی باشند ══
        # شناسهٔ یک کش از کلید **و فهرستِ مسیرها** ساخته می‌شود. اگر
        # ذخیره و بازیابی یک مسیر اختلاف داشته باشند، کلید جور است و کش
        # باز هم پیدا نمی‌شود: هر اجرا از صفر، بی هیچ خطایی. همین نکته
        # بود که کلیدِ «مهاجرت» را ناممکن کرد.
        eq(len({str(x["with"]["path"]).strip() for x in got}), 1,
           "و فهرستِ مسیرهاشان هم یکی است — وگرنه شناسهٔ کش جور نمی‌شود")

    bkey = str(fam["base"][0]["with"]["key"])
    ckey = str(fam["ckpt"][0]["with"]["key"])
    eq("github.run_id" in ckey and "github.run_id" not in bkey, True,
       "کلیدِ سبک هر اجرا تازه است، کلیدِ سنگین نیست (%s / %s)"
       % (ckey, bkey))
    # ══ و کلیدِ سنگین نباید عددِ دست‌نویس باشد ══
    # یک `-v1`ِ دستی یعنی روزی که ضبطِ تازه‌ای اضافه شود، کد دیتاست را
    # درست از نو می‌سازد و کش تا ابد کهنه را برمی‌گرداند، چون کسی یادش
    # نمی‌مانَد عدد را جلو ببرد. `dsSig_()` همان تابعی است که خودِ کد با
    # آن همین تصمیم را می‌گیرد — یک منبع، نه دو.
    eq("steps.sig.outputs.sig" in bkey, True,
       "کلیدِ سنگین به اثرِ انگشتِ ورودی بسته است، نه به عددِ دستی")
    eq("dsSig_" in wf, True, "و آن اثرِ انگشت از همان تابعِ کد می‌آید")

    # ══ دو کش که روی هم بیفتند یعنی هنوز یک کشِ تمام-درخت ══
    # کلِ فایده این است که سنگین **نوشته نشود**. اگر مسیری هم در سبک
    # باشد و هم در سنگین، سنگین هر اجرا عوض می‌شود و باز یک نسل بیشتر
    # جا نمی‌شود — همان حالتی که ۱۲ سپتامبر بیست‌وسه دور را برد.
    apaths = [x.strip() for x in
              str(fam["base"][0]["with"]["path"]).splitlines() if x.strip()]
    bpaths = [x.strip() for x in
              str(fam["ckpt"][0]["with"]["path"]).splitlines() if x.strip()]

    # ══ و کنار گذاشتن با `!` **کار نمی‌کند** — با عدد ثابت شد ══
    # نسخهٔ اول دستهٔ سنگین را «کلِ درخت، منهای اینها» تعریف کرده بود.
    # اجرای ۴۵ حکم داد: درخت روی دیسک ۱۱ گیگ، و کشِ «فیلترشده» ۱۰٫۱۱ گیگِ
    # فشرده. اگر آن خط‌ها گرفته بودند حداکثر ~۷٫۷ گیگِ خام می‌مانْد و
    # فشرده‌اش کمتر — پس هیچ‌کدام هرگز چیزی را کنار نگذاشته بود، و شش خطِ
    # بی‌اثر بالای یک کشِ تمام-درخت نشسته بود. الگوی مثبت در همین فایل
    # اثباتش را دارد: کشِ چک‌پوینت‌ها با همین شکل هر بار درست برمی‌گردد.
    eq([x for x in apaths if x.startswith("!")], [],
       "کشِ سنگین فقط الگوی مثبت دارد — `!` در actions/cache نمی‌گیرد")
    eq([x for x in bpaths if x.startswith("!")], [],
       "و سبک هم همین‌طور")

    # ══ ناهمپوشانی، حالا که هر دو مثبت‌اند ══
    # هیچ الگوی سنگینی نباید چیزی را بگیرد که سبک می‌گیرد. چون هر دو
    # فهرستِ صریح‌اند، کافی است هیچ نامِ مشترکی نداشته باشند.
    for b in bpaths:
        eq(b in apaths, False, "مسیرِ سبکِ %s در سنگین نیست" % b)
    for tail in ("G_*.pth", "D_*.pth", "assets/weights", "assets/indices",
                 "/out"):
        eq([x for x in apaths if tail in x], [],
           "و سنگین چیزی از جنسِ %s برنمی‌دارد" % tail)

    # ══ و میان‌بُرها هم در آن نیستند ══
    # `raw`/`dataset`/`1_16k_wavs` فقط ورودیِ ساختِ ویژگی‌هایند و
    # `preTrain_` نمی‌خواندشان. حالا که فهرست مثبت است، «نبودن» یعنی
    # واقعاً نبودن — نه یک خطِ `!` که هیچ‌وقت نگرفت.
    for mid in ("dataset", "1_16k_wavs"):
        eq([x for x in apaths if mid in x], [],
           "میان‌بُرِ %s در کشِ سنگین نیست" % mid)
    # ══ ولی `raw` **هست**، و معیارش اندازه نیست ══
    # اول با بقیه بیرونش گذاشتم چون آن هم «میان‌بُر» است. اجرای ۴۶ نشان
    # داد معیار اشتباه بوده: ~۲۰۰ مگ است (در برابرِ ۱٫۱ گیگِ dataset) و
    # بیرون بودنش یعنی هر بازسازیِ پایه هشت فایل را از گوگل‌درایو دوباره
    # می‌کشد — که همان‌جا به سهمیه خورد و اجرا را کشت. چیزی که ارزان است
    # و نبودنش یک وابستگیِ بیرونی را هر بار صدا می‌زند، باید بمانَد.
    eq([x for x in apaths if x.endswith("/raw")] != [], True,
       "ولی ضبط‌های خام در کش می‌مانند — تا درایو هر بار صدا زده نشود")

    # ══ و آنچه هست، همان است که آموزش می‌خواند ══
    # `rvcpipe.preTrain_` فهرست را از این چهار پوشه می‌سازد. اگر روزی
    # یکی‌شان از کش بیفتد، هر اجرا دو ساعت‌ونیم آن را دوباره می‌سازد —
    # بی هیچ خطایی، چون کد فقط کندتر می‌شود.
    pipe = io.open(os.path.join(os.path.dirname(os.path.abspath(
        VT.__file__)), "rvcpipe.py"), encoding="utf-8").read()
    for d in ("0_gt_wavs", "3_feature768", "2a_f0", "2b-f0nsf"):
        eq(d in pipe, True, "%s را خودِ زنجیره می‌شناسد" % d)
        eq([x for x in apaths if d in x] != [], True,
           "و کشِ سنگین %s را نگه می‌دارد" % d)

    # ══ کشِ یک‌بارنوشت، اگر نیمه‌کاره پر شود، تا ابد نیمه‌کاره است ══
    # پس نوشتنش دو شرط دارد و هر دو لازم‌اند: از قبل نباشد، و این اجرا
    # شاهدِ کامل بودن داده باشد (`baseReady_`). شرطِ تهی یعنی «نه».
    bsave = [x for x in fam["base"] if "save" in str(x["uses"])][0]
    eq("cache-hit != 'true'" in str(bsave.get("if", "")), True,
       "کشِ سنگین اگر باشد دوباره نوشته نمی‌شود")
    eq("base_ok == 'true'" in str(bsave.get("if", "")), True,
       "و فقط با شاهدِ کامل بودن نوشته می‌شود")

    # ══ restore-key فقط باید همان خانواده را ببیند ══
    rk = [x.strip() for x in
          str(fam["ckpt"][0]["with"]["restore-keys"]).splitlines() if x.strip()]
    eq(rk and all(ckey.startswith(x) for x in rk), True,
       "هر کلیدِ پشتیبان پیشوندِ همان کلید است: %s" % rk)
    # و این بندِ دوم، چون بندِ اول تنها کافی نیست: `rvc-razavi-`ِ خالی
    # پیشوندِ کلیدِ سبک **هم هست**، پس از شرطِ بالا رد می‌شود. مرزِ
    # واقعی این است که کلیدِ پشتیبان نباید خانوادهٔ دیگر را هم بگیرد.
    eq([x for x in rk if bkey.startswith(x)], [],
       "و هیچ‌کدام کشِ سنگین را نمی‌گیرد: %s" % rk)

    # ══ عددی که هر روز خوانده می‌شود باید آخرین خط باشد ══
    # APIِ لاگِ گیت‌هاب فقط دُم می‌دهد، و آن دُم را چند صد خطِ
    # «Sent N of N»ِ بارگذاریِ کش پر می‌کند. تا ۱۵ سپتامبر پیدا کردنِ
    # «دورِ چندم» چند بار کشیدنِ لاگ لازم داشت و آخرش هم نیامد. پس
    # مرحلهٔ «خلاصه» آخر از همه است — اگر روزی مرحله‌ای زیرش اضافه شود،
    # همان هزینه بی‌صدا برمی‌گردد.
    eq(steps_[-1]["name"], "خلاصه",
       "مرحلهٔ خلاصه آخرِ همه است (الان: %s)" % steps_[-1]["name"])
    eq("state.json" in str(steps_[-1]["run"]), True,
       "و عددش را از state.json می‌خوانَد، نه از لاگ")

    # ══ نردبان: تنها بخشِ رشدکنندهٔ کش ══
    # `G_*`/`D_*` اندازهٔ ثابت دارند؛ `assets/weights/<voice>_e<N>.pth` هر
    # دور یکی اضافه می‌کند. و سقفِ ۱۰ گیگ در **لحظهٔ ذخیره** سنجیده
    # می‌شود، جایی که نسلِ کهنه و تازهٔ چک‌پوینت هر دو هستند — یعنی
    # `پایه + ۲×چک‌پوینت`. اجرای ۵۱ از همین رد شد و گیت‌هاب کهنه‌ترین کش
    # را انداخت: پایه را، چون یک‌بار نوشته می‌شود و هرگز تازه نمی‌شود.
    # ۵۲ دقیقه بازسازی، و آن اجرا به‌جای دو دور یکی داد.
    lad = [x for x in steps_ if str(x.get("id", "")) == "ladder"]
    eq(len(lad), 1, "مرحلهٔ بایگانیِ نردبان شناسه دارد تا بشود به آن شرط بست")
    iL = steps_.index(lad[0])
    iPs = [i for i, x in enumerate(steps_) if "ladderPrune_" in str(x.get("run", ""))]
    eq(len(iPs), 1, "کوتاه کردنِ نردبان یک‌جا انجام می‌شود (%d)" % len(iPs))
    iP = iPs[0]
    # ══ ترتیب، و شرطی که کلِ امنیتِ کار به آن است ══
    # این عکس‌ها بی‌خود ساخته نشده‌اند: یافتنِ دوری که مدل از آن به بعد
    # به‌جای یاد گرفتنِ گوینده داده را حفظ می‌کند، فقط با مقایسهٔ
    # همین‌هاست. پس پاک نمی‌شوند، **جا عوض می‌کنند** — و حذف به سبز شدنِ
    # بایگانی بسته است. حذفی که بایگانی‌اش نرفته باشد برنمی‌گردد.
    eq(iL < iP, True, "اول بایگانی، بعد حذف (%d < %d)" % (iL, iP))
    eq("steps.ladder.outcome == 'success'" in str(steps_[iP].get("if", "")), True,
       "و حذف فقط وقتی اجرا می‌شود که بایگانی سبز شده باشد")
    iC = [i for i, x in enumerate(steps_)
          if x in fam["ckpt"] and "save" in str(x.get("uses", ""))]
    eq(len(iC) == 1 and iP < iC[0], True,
       "و کوتاه کردن پیش از ذخیرهٔ کشِ سبک است — بعدش بی‌فایده بود")

    # ══ و خودِ تابع: بالاترین، نه آخرینِ فهرست ══
    # مرتب‌سازیِ نامی `_e9` را بعد از `_e10` می‌گذارد. تا دورِ ۹ هیچ‌کس
    # نمی‌فهمید؛ از ۱۰ به بعد هر اجرا وزنِ تازه را می‌انداخت و یکی از
    # کهنه‌ها را نگه می‌داشت — بی هیچ خطایی، و فقط با گوش شنیدنی.
    lroot = os.path.join(tempfile.mkdtemp(), "ladder")
    wdir_ = os.path.join(lroot, "assets", "weights")
    gdir_ = os.path.join(lroot, "logs", VT.VOICE)
    for dd in (wdir_, gdir_):
        os.makedirs(dd)
    for n in range(1, 20):
        io.open(os.path.join(wdir_, "%s_e%d.pth" % (VT.VOICE, n)), "w").write("x")
    io.open(os.path.join(wdir_, "%s.pth" % VT.VOICE), "w").write("x")
    for g in ("G_2333333.pth", "D_2333333.pth"):
        io.open(os.path.join(gdir_, g), "w").write("x")
    was = VT.epochsDone_(lroot)
    gone = VT.ladderPrune_(lroot)
    left = sorted(os.listdir(wdir_))
    eq(left, ["%s.pth" % VT.VOICE, "%s_e19.pth" % VT.VOICE],
       "بالاترین می‌مانَد و وزنِ نهایی دست نمی‌خورد (%s)" % left)
    eq(len(gone), 18, "و هجده‌تای دیگر می‌روند (%d)" % len(gone))
    # ══ عددی که هر روز خوانده می‌شود نباید عقب برود ══
    # `epochsDone_` **بیشینه** می‌گیرد، نه شمارش — پس تا وقتی بالاترین
    # بمانَد، خطِ «دورِ چندم» همان است که بود. اگر روزی کسی آن را به
    # شمارش عوض کند، کوتاه کردنِ نردبان بی‌صدا ۱۹ را می‌کرد ۱.
    eq((was, VT.epochsDone_(lroot)), (19, 19),
       "و `epochs_reached` تکان نمی‌خورد")
    # ══ ازسرگیری به `G_`/`D_` بسته است — خطِ قرمز ══
    eq(sorted(os.listdir(gdir_)), ["D_2333333.pth", "G_2333333.pth"],
       "و چک‌پوینتِ ازسرگیری دست‌نخورده می‌مانَد")
    # بی‌اثر بودنِ اجرای دوباره: مرحله `always()` دارد و ممکن است در
    # اجرایی که آموزش چیزی نساخت هم بدود.
    eq(VT.ladderPrune_(lroot), [], "بارِ دوم چیزی برای بردن نیست")
    # ══ و کفِ یک، که نشود اشتباهی از آن رد شد ══
    # `keep=0` هم وزنِ قابلِ‌استفاده را می‌بَرد و هم عدد را صفر می‌کند.
    eq(VT.ladderPrune_(lroot, keep=0), [], "با keep=0 هم بالاترین می‌مانَد")
    eq(VT.epochsDone_(lroot), 19, "و عدد باز هم همان است")
    ...
    # ══ restore-key فقط باید همان خانواده را ببیند ══
    # یک `rvc-razavi-`ِ خالی هم کشِ سنگین را می‌گیرد هم سبک را، و آن
    # وقت «کارِ اجرای پیشین برگشت» می‌گوید در حالی که هیچ چک‌پوینتی
    # نیامده — یعنی نگهبانِ پایین را با یک کشِ بی‌ربط دور می‌زند.
    rk = [x.strip() for x in
          str(fam["ckpt"][0]["with"]["restore-keys"]).splitlines() if x.strip()]
    eq(rk and all(ckey.startswith(x) for x in rk), True,
       "هر کلیدِ پشتیبان پیشوندِ همان کلید است: %s" % rk)
    ...
    # پیشوندِ کلیدِ سبک **هم هست**، پس از شرطِ بالا رد می‌شود. مرزِ
    # واقعی این است که کلیدِ پشتیبان نباید خانوادهٔ دیگر را هم بگیرد.
    # (همین را با آزمونِ جهش سنجیدم: بندِ اول تنها، جهش را نگرفت.)
    eq([x for x in rk if bkey.startswith(x)], [],
       "و هیچ‌کدام کشِ سنگین را نمی‌گیرد: %s" % rk)
    ...
    # ══ دادهٔ تازه «ادامه» نیست (۷ سپتامبر) ══
    # صاحبِ برنامه گفت «۳۲ دور آموزش را شروع کن». پشتِ آن یک جمله، سه
    # دامِ خاموش بود و هر سه از یک جنس‌اند — کشی که ورودی‌اش عوض شده:
    #   • `dataset/` اگر خالی نباشد اصلاً دوباره ساخته نمی‌شود، پس هشت
    #     ضبطِ تازه دانلود می‌شدند و دیتاستِ ۳۹ دقیقه‌ای می‌ماند؛
    #   • چک‌پوینتِ ۲۰۰ دور روی دادهٔ قدیمی است و ادامه‌اش روی دادهٔ
    #     تازه معنایی ندارد؛
    #   • و `state.json` هدفِ ۲۰۰ را ثبت کرده، پس درخواستِ ۳۲ دور در
    #     چند ثانیه با «از پیش کامل شده» برمی‌گشت — سبز، و بی آموزش.
    # جوابش یک عادتِ آدمی نیست، یک اثرِ انگشت است.
    eq(len(VT.DRIVE_IDS) == 8 and len(set(VT.DRIVE_IDS)) == 8, True,
       "هر هشت ضبط در فهرست‌اند و تکراری نیست (%d)" % len(VT.DRIVE_IDS))
    sig0 = VT.dsSig_()
    keep = VT.DRIVE_IDS[:]
    VT.DRIVE_IDS = keep[:-1]
    eq(VT.dsSig_() != sig0, True, "کم‌شدنِ یک ضبط اثرِ انگشت را عوض می‌کند")
    VT.DRIVE_IDS = list(reversed(keep))
    eq(VT.dsSig_() == sig0, True,
       "ولی فقط جابه‌جاییِ ترتیب، بازسازیِ بی‌دلیل نمی‌سازد")
    VT.DRIVE_IDS = keep
    capWas = VT.DS_MAX_MIN
    ...
    # ══ فاصلهٔ ذخیره باید کوچک‌تر از بودجهٔ یک اجرا باشد (اجرای #۱۳) ══
    # ۵ دور روی دیتاستِ ۳۹ دقیقه‌ای ۲٫۲ ساعت بود و در بودجهٔ ۴٫۸ ساعته
    # جا می‌شد؛ روی ۲۴۰ دقیقه ۱۱٫۷ ساعت شد. اجرای ۱۳ چهار ساعت آموزش
    # داد، به دورِ ۲ رسید و **صفر چک‌پوینت** نوشت — و اجرای بعدی از صفر
    # شروع می‌کرد. حلقه‌ای سبز و بی‌پایان.
    eq(VT.SAVE_EVERY, 1, "هر دور یک ذخیره (%d)" % VT.SAVE_EVERY)
    # و اگر باز هم گامِ صفر شد، جملهٔ «از همین‌جا ادامه می‌دهد» — که آن
    # شب دروغ بود — نباید نوشته شود.
    eq('elif not st["steps"]:' in src29 and "گامِ صفر" in src29, True,
       "گامِ صفر هشدارِ خودش را دارد، نه جملهٔ «ادامه می‌دهد»")
    ...
    # وزن‌های پایه به ورودی ربطی ندارند و آوردنشان گران است — نباید بروند.
    fs = src29.split("def freshStart_", 1)[1].split("\ndef ", 1)[0]
    eq('"assets"' not in fs.replace('"assets", "indices"', ""), True,
       "وزن‌های پایه در پاک‌سازی نمی‌روند")
    ...
    # تازه یعنی یک آدم فایل را دستی بردارد و در درایو بگذارد — و سنجشی
    # که به کارِ دستی بند باشد، همان سنجشی است که انجام نمی‌شود. همان
    # شکلِ بانکِ موسیقی که هفته‌ها خالی ماند.
    steps_ = _yamlStrict_(wfl)["jobs"]["lab"]["steps"]
    names_ = [str(x.get("name", "")) for x in steps_]
    iA = [i for i, x in enumerate(steps_) if "gh run download" in str(x.get("run", ""))]
    iD = [i for i, x in enumerate(steps_) if "ref$nref.input" in str(x.get("run", ""))]
    eq(len(iA) == 1 and len(iD) == 1, True,
       "یک مرحله از artifact می‌گیرد و یک مرحله از درایو (%s / %s)" % (iA, iD))
    # **پیش از** مرحلهٔ درایو، وگرنه درایو اول می‌دود و خطا می‌دهد که
    # خانه‌اش خالی است، و artifact هیچ‌وقت نوبتش نمی‌رسد.
    eq(iA[0] < iD[0], True, "اول artifact، بعد درایو (%d < %d)" % (iA[0], iD[0]))
    # و مرحلهٔ درایو نباید روی فایلی که آمده بنویسد — وگرنه پر بودنِ
    # خانهٔ درایو بی‌صدا مدلِ artifact را دور می‌ریزد و عددی که گزارش
    # می‌شود مالِ مدلِ دیگری است. بدترین شکل: سنجشِ درست روی مدلِ غلط.
    drv = str(steps_[iD[0]].get("run", ""))
    for f in ("model.pth", "model.index"):
        eq(("[ ! -f %s ]" % f) in drv, True,
           "درایو روی %s ِ آمده از artifact نمی‌نویسد" % f)
    # نامِ فایل از خودِ پوشه خوانده شود، نه فهرستِ دستی — درسِ ref*.input
    art = str(steps_[iA[0]].get("run", ""))
    eq("find vrart" in art, True, "نامِ فایل‌ها از پوشه خوانده می‌شود")
    eq("razavi.pth" not in art, True, "و نامِ مدل دستی نوشته نشده")
    ...
    # ══ و از ۱۳ سپتامبر، هشدار کافی نیست: باید بایستد ══
    # اجرای ۳۹ همین هشدار را داد و بعد از صفر شروع کرد و ۲۳ دورِ آموزش
    # (نزدیک شصت ساعت) را دور ریخت — و کشِ دورِ ۱ را روی آن نوشت.
    eq("::error title=کش برنگشت::" in wft, True,
       "نبودنِ کش اجرا را می‌خوابانَد، نه اینکه فقط هشدار بدهد")
    eq("allow_fresh" in wft, True,
       "و شروعِ عمدی از صفر فقط با دستِ آدم ممکن است")
    eq("steps.train.conclusion != 'skipped'" in wft, True,
       "اجرایی که نخوابیده باشد کشِ خالی نمی‌نویسد")
    eq("id: cache" in wft, True, "و مرحلهٔ برگرداندن شناسه دارد تا بشود پرسید")
    # و دو اجرای هم‌زمان: کش در پایانِ اجرا نوشته می‌شود، پس اجرایی که
    # پیش از تمام‌شدنِ قبلی راه بیفتد چیزی نمی‌گیرد و از صفر شروع می‌کند.
    eq("concurrency:" in wft and "group: voice-train" in wft, True,
