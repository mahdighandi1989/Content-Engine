...
def main():
    ...
    eq(scSrc0.count("stylePitch_(cat, rate)"), 1,
       "و فقط یک جا از خودِ نمونه‌ها می‌سازدش")
    body35 = "\n".join(ln for ln in scSrc0.splitlines()
                       if not ln.startswith("def "))
    eq("stylePitch_(y, rate)" not in body35, True,
       "هیچ‌جا دیگر روی کلِ فایل (با سکوت) حساب نمی‌شود")
    ...
    eq("offset" in SC.modeSamples_.__code__.co_varnames, True,
       "برشِ نمونه جابه‌جاییِ پنجره را می‌گیرد")
    eq("window_offset_seconds" in scSrc0, True, "و در خروجی ثبت می‌شود")
