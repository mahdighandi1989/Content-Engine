...
def main():
    ...
    # ══ شکستِ موتور باید قرمز باشد ══
    # اینجا بی‌قید `return 0` بود، با این استدلال که «شکستِ یک موتور،
    # شکستِ آزمایش نیست: خودِ خبر همان چیزی است که می‌خواستیم». استدلال
    # درست است، ولی سدّش **جای دیگری** است: ماتریسِ گردش‌کار
    # `fail-fast: false` دارد، پس هر موتور job خودش است و شکستش بقیه را
    # نمی‌کُشد. `return 0` چیزی به آن اضافه نمی‌کرد جز پنهان کردنِ شکست.
    #
    # ۱۸ سپتامبر، اجرای ۶۱: `engine=style` روی هشت ضبطِ رضوی **سبز**
    # تمام شد و هیچ کارتی نساخت — `onnxruntime` نبود، خطا در گزارش ثبت
    # شد، و job «موفق» اعلام شد. نزدیک بود بر پایهٔ همان سبز به صاحبِ
    # برنامه گفته شود «کارت ساخته شد». گزارشی که فقط در فایل بگوید
    # شکست خورده و در **وضعیتِ اجرا** نه، همان «سبز و بی‌خطا و بی‌اثر»
    # است که این مخزن یک بخشِ کامل دربارهٔ آن دارد.
    if rep.get("error"):
        print("::error title=موتور %s شکست خورد::%s"
              % (a.engine, str(rep["error"])[:200]), flush=True)
        return 1
    return 0
