...
# چهل‌دقیقه‌ایِ دیگر.
CANDIDATES = [
    # ══ تشخیصِ گوینده، نه تشخیصِ گفتار ══
    # بازخوردِ کاربر روی نمونهٔ فیلتر: در تکه‌های نگه‌داشته، تیزرِ برنامه
    # با صدای یک زن و یک مردِ دیگر هم آمده بود. VAD می‌گوید «اینجا گفتار
    # هست» و **نمی‌گوید گفتارِ چه کسی** — و من فرض کرده بودم هرچه گفتار
    # است رضوی است. آموزش روی صدای شخصِ دیگر، مدل را خراب می‌کند.
    #
    # `resemblyzer` (Apache-2.0) گزینهٔ سبک بود ولی دو ایراد داشت که فقط
    # با خواندن معلوم شد: هنگام import به `webrtcvad` نیاز دارد، و از
    # `scipy.ndimage.morphology` استفاده می‌کند که در SciPyهای تازه حذف
    # شده. پس سراغِ چیزی می‌رویم که **هیچ وابستگیِ تازه‌ای نمی‌خواهد**:
    # `WavLMForXVector` در transformers ۴٫۴۹ هست (خوانده شد) و torch هم
    # از قبل نصب است. می‌مانَد پروانهٔ وزن.
    # ══ و جوابِ اسکنِ #۴۵: پروانه‌اش **اعلام نشده** ══
    # کدِ WavLM در `microsoft/unilm` ‏MIT است، ولی چک‌پوینتِ تنظیم‌شدهٔ
    # روی HF هیچ اعلامی ندارد. همان تفکیکی که در RVC ثبت شد: پروانهٔ
    # **وزن** تصمیم می‌گیرد، نه پروانهٔ کد. پس نامعلوم می‌مانَد و برای
    # کانالی که قرار است درآمد داشته باشد به کار نمی‌رود.
    "microsoft/wavlm-base-plus-sv",
    # جایگزینی که پروانه‌اش را صریح اعلام می‌کند — همین را می‌سنجیم.
    "speechbrain/spkrec-ecapa-voxceleb",
    # ══ RVC: کد MIT است، ولی وزن‌ها جای دیگری‌اند ══
    # `Retrieval-based-Voice-Conversion-WebUI` پروانهٔ MIT دارد — خواندمش.
    # ولی هیچ‌کدام از وزن‌ها در آن مخزن نیستند: مدلِ پایه، رمزگذارِ محتوا
    # (hubert_base) و استخراج‌گرِ زیروبمی (rmvpe) همه از این یک مخزنِ HF
    # می‌آیند. پروانهٔ **آن** تعیین می‌کند می‌شود استفاده کرد یا نه —
    # همان تفکیکی که XTTS و KiaBush را از دور خارج کرد.
    # ══ جواب (اجرای #۳۵): زنجیره تمیز است ══
    #   کدِ RVC ......................... MIT (خودِ فایلِ LICENSE)
    #   lj1995/VoiceConversionWebUI ..... MIT (اعلامِ خودِ صاحبانِ وزن)
    #   ContentVec (رمزگذارِ محتوا) ...... MIT (auspicious3000/contentvec)
    #   RMVPE (استخراجِ زیروبمی) ......... Apache-2.0 (Dream-High/RMVPE)
    # و یک نکتهٔ باریک که باید نوشته شود نه کشف: وزنِ rmvpe را انشعابِ
    # yxlllc آموزش داده و آن انشعاب هیچ پروانه‌ای در ریشه‌اش ندارد.
    # بازتوزیع‌کننده (lj1995) آن را زیرِ MIT پخش می‌کند، و مهم‌تر:
    # rmvpe **اختیاری** است — RVC با pm/harvest/crepe هم کار می‌کند.
    # پس تنها قطعهٔ مبهم، تنها قطعهٔ قابلِ‌تعویض هم هست.
