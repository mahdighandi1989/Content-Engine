...
# ══ «سرعتِ ادا» اندازه گرفته نمی‌شود — و چرا ══
# صاحبِ برنامه دو نمونهٔ حالت را شنید و درست گفت: «۲ تندتر می‌خوانَد و
# ۱ با طمأنینهٔ بیشتر». این محور در بردارِ ما نیست، پس سه سنجه برایش
# ساختم و هر سه را با یک محکِ ساده آزمودم — **صوت را ۱٫۵ برابر تند
# کن؛ عدد باید ۱٫۵ برابر شود**:
#
#   librosa.onset_detect            ×1.5 → ×0.82   (وارونه!)
#   شارِ طیفی (دلتای MFCC)          ×1.5 → ×1.08   (اشباع)
#   هسته‌های هجایی (De Jong)        ×1.5 → ×1.25   (نصفِ مقیاس)
#
# هیچ‌کدام آنچه ادعا می‌کند را نمی‌سنجد. سنجهٔ سوم جهت را درست
# می‌گیرد ولی مقیاسش نصف است، یعنی عددِ مطلقش دروغ است. و شارِ طیفی
# روی همان دو نمونه **برعکس** جواب داد. سه ابزار که با هم نمی‌خوانند،
# یعنی شاهدی در کار نیست.
#
# پس عددی برایش در کارت نمی‌نشیند. قاعدهٔ این پرونده «هر جمله از یک
# عدد» است، و عددی که چیزِ دیگری را می‌سنجد از نبودنش بدتر است.
#
# و احتمالِ قوی‌تر این است که آنچه او شنید همان چیزی باشد که **داریم**
# می‌سنجیم: شنونده معمولاً «تندی» را از تراکمِ مکث می‌فهمد نه از سرعتِ
# هجا، و همان دو حالت ۱۵ در برابرِ ۲۳ مکث در دقیقه دارند.
#
# راهِ درستش رونویسِ خودکار است (هجا در ثانیهٔ گفتار، با متنِ واقعی) —
# یعنی یک مدلِ ASR، که هزینه و وابستگیِ تازه دارد. اگر روزی لازم شد،
# از اینجا شروع کنید، نه از یک تخمینِ دیگر.
