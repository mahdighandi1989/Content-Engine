...
def _word(w, m):
    ...
    while i < n:
        ...
        if ch in CONS:
            ...
            put("c", ch)
            i += 1
            # تشدید و اعرابِ کوتاه هر دو ترتیب نوشته می‌شوند: «ذّ» + «َ» و
            # «ذ» + «َ» + «ّ». تا امروز فقط اولی دیده می‌شد و «لَذَّت» به
            # lazat بدل می‌شد — یعنی تشدید بی‌صدا گم می‌شد.
            if i + 1 < n and w[i] in short and w[i + 1] == SHADDA:
                w = w[:i] + SHADDA + w[i] + w[i + 2:]
...
def _join(toks, m):
    ...
    if m != 1:
        ...
        for i, (k, t) in enumerate(toks):
            ...
            # ══ قراردادِ اضافه، از نمونه‌های خودِ مدل ══
            # `examples/metadata.json` می‌گوید «بابتِ این» را `bɒːbæte ʔiːn`
            # می‌نویسد: کسرهٔ اضافه به واژه **چسبیده** و خط‌تیره‌ای در کار
            # نیست. خط‌تیره برای خواندنِ آدم خوب است و برای این مدل یک
            # نویسهٔ اضافه — و ما تا امروز حدس زده بودیم.
            sep = "" if m == 2 else "-"
            out.append(sep + (("y" if m == 0 else "j") if glide else "") + t)
        s2 = "".join(out)
        # ══ همزهٔ آغازِ واژه ══
        # همان فایل: «این» → `ʔiːn`، «از» → `ʔæz`، «آن» → `ʔɒːn`. یعنی هر
        # واژه‌ای که با مصوت شروع شود، در قراردادِ این مدل یک ʔ می‌گیرد.
        if m == 2 and toks and toks[0][0] in ("v", "z") and not s2.startswith("ʔ"):
            s2 = "ʔ" + s2
        return s2
