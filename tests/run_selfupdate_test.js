   ...
   روز بی‌صدا کارِ اشتباه می‌کند. بیانیه باید بتواند این را اعلام کند.        */
{
  ...
  /* از ۵٫۹۱ این یادآور هر شب ایمیلِ خودش را نمی‌گیرد (هر شب تا وقتی انجام
     شود، یعنی روزها ایمیلِ تکراری) — در گزارشِ روزانه با بقیه می‌آید. */
  ok('۱۲.۴ خبرش در گزارشِ روزانه می‌رود، نه ایمیلِ هرشبه',
     mailQueueRead_().some(x => /دستورِ روتین‌ها/.test(x.title)) && mail.length === 0,
  ...
  /* تا ۵٫۹۲ این متن می‌گفت دستورها «بیرونِ ریپو زندگی می‌کنند» و «دستی»
     باید به‌روز شوند. از ۵٫۸۵ در ریپو زندگی می‌کنند و promptSyncFromRepo_
     هر شب می‌آوردشان — پس آن جمله غلط شده بود و این سنجه غلط را نگه
     می‌داشت. آنچه نباید عوض شود: پیام باید بگوید **چه کاری مانده**. */
  ok('۱۲.۶ و می‌گوید کارِ لازم کجاست: نسخهٔ تازه در ریپو',
     /docs\/prompts/.test(tg[0]) && /push/.test(tg[0]), tg[0].slice(0, 200));
  ok('۱۲.۶-ب و می‌گوید از صاحبِ برنامه کاری برنمی‌آید',
     /کاری از شما لازم نیست/.test(tg[0]));
