...
{
  /* ══ گزارشِ ۳ سپتامبر (۶٫۸۹) و بازگشتش در ۷ سپتامبر (۶٫۹۷) ══
   * «داوریِ ارجاع‌ها: ۰ ارجاع داوری شده · ۱۱ در صف. **هیچ ارجاعی تا امروز
   * داوری نشده**.» ۶٫۸۹ برایش رزرو ساخت — شبی یک کار، با سقفِ ۲۵٪ بودجه —
   * و چهار روز بعد هشت کار گرسنه بودند و جزوه دو روز دست‌نخورده مانده بود.
   *
   * عددها می‌گویند چرا: بودجه ۲۷۰ ثانیه است و جمعِ **کف**ِ نگهبان‌ها ۶۲۰
   * ثانیه. کسری از جنسِ ۲٫۳ برابر را با «شبی یک نوبت» نمی‌شود بست.
   * جواب تمام‌کردنِ کار در چند اجراست، نه بزرگ‌کردنِ سهمِ یکی. */
  ...
  // ── اجرای اول: بودجه سرِ بلوکِ دوم تمام می‌شود ──
  let q = quiet();
  let n1 = nightBegin_();
  ok('۳۲.۱ نخستین اجرای شب، کارهای ارزان را می‌دود', n1.first === true);
  const huge = (Number(CFG.NIGHT_BUDGET_MS) || 270000) + 60000;
  const a1 = nightHas_(1000, 'کارِ یک');
  const a2 = nightHas_(huge, 'کارِ دو');
  const a3 = nightHas_(1000, 'کارِ سه');
  const e1 = nightEnd_(n1.runs);
  q();
  ok('۳۲.۲ بلوکِ اولِ ارزان اجرا می‌شود', a1 === true);
  ok('۳۲.۳ بلوکی که جا نشد متوقف می‌شود', a2 === false);
  /* و بلوکِ بعدی هم — چون اگر بلوکِ ارزانِ پس از آن اجرا می‌شد، ترتیبِ
     اهمیت به‌هم می‌خورد و ادامه نمی‌دانست از کجا شروع کند. */
  ok('۳۲.۴ و بلوک‌های بعدی هم صبر می‌کنند، نه اینکه از رویش بپرند', a3 === false);
  ok('۳۲.۵ شب تمام‌نشده اعلام می‌شود', e1.done === false);
  const cur = JSON.parse(P[PK.NIGHT_STEP] || 'null');
  ok('۳۲.۶ و جای توقف نوشته می‌شود', cur && cur.next === 'کارِ دو' && cur.runs === 1,
     JSON.stringify(cur));
  /* و هیچ گرسنگی‌ای ثبت نمی‌شود: کاری که ادامه‌اش زمان‌بندی شده، گرسنه
     نیست. پیش از ۶٫۹۷ همین‌جا برای هر بلوکِ نرسیده یک شب شمرده می‌شد و
     خطِ سلامت هر شب چیزی می‌گفت — که یعنی چیزی نمی‌گفت. */
  ok('۳۲.۷ کارِ نوبت‌دارشده گرسنه شمرده نمی‌شود',
     !Object.keys(JSON.parse(P[PK.NIGHT_STARVE] || '{}')).length,
     P[PK.NIGHT_STARVE]);

  // ── اجرای دوم همان شب: از همان‌جا، و ارزان‌ها دوباره نمی‌دوند ──
  q = quiet();
  const n2 = nightBegin_();
  const b1 = nightHas_(1000, 'کارِ یک');
  const b2 = nightHas_(1000, 'کارِ دو');
  const b3 = nightHas_(1000, 'کارِ سه');
  const e2 = nightEnd_(n2.runs);
  q();
  ok('۳۲.۸ ادامه، اجرای دومِ همان شب است', n2.first === false && n2.runs === 1);
  ok('۳۲.۹ بلوکِ انجام‌شده دوباره اجرا نمی‌شود', b1 === false);
  ok('۳۲.۱۰ و از همان‌جا که ماندیم ادامه می‌دهد', b2 === true && b3 === true);
  ok('۳۲.۱۱ فهرست که تمام شد، مکان‌نما پاک می‌شود',
     e2.done === true && !P[PK.NIGHT_STEP]);

  // ── سقفِ اجراها: حلقه‌ای که خودش را زمان‌بندی می‌کند باید ته داشته باشد ──
  q = quiet();
  delete P[PK.NIGHT_STARVE];
  const max = Math.max(1, Number(CFG.NIGHT_MAX_RUNS) || 8);
  nightBegin_();
  nightHas_(huge, 'کارِ دو');
  const eMax = nightEnd_(max - 1);
  q();
  ok('۳۲.۱۲ پس از سقفِ اجراها شب بسته می‌شود، نه اینکه تا ابد ادامه بدهد',
     eMax.done === false && !P[PK.NIGHT_STEP]);
  ok('۳۲.۱۳ و **همان‌جا** گرسنگی شمرده می‌شود — چیزی که با ادامه‌ها هم نشد',
     (JSON.parse(P[PK.NIGHT_STARVE] || '{}')['کارِ دو'] || {}).n === 1,
     P[PK.NIGHT_STARVE]);
  const st = nightStarveStatus_();
  ok('۳۲.۱۴ و در سلامت اعلام می‌شود', /کارِ دو/.test(st.line), st.line);

  // ── مکان‌نمای دیشب، امشب معتبر نیست ──
  q = quiet();
  P[PK.NIGHT_STEP] = JSON.stringify({ day: '2000-01-01', next: 'کارِ دو', runs: 3 });
  const n3 = nightBegin_();
  q();
  ok('۳۲.۱۵ مکان‌نمای روزِ دیگر دور ریخته می‌شود، وگرنه شبِ تازه از وسط شروع می‌کند',
