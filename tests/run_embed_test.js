 ...
 */
{
  ...
  /* ══ ۲۰.۴-ب سقفِ دنباله از مسیرِ **دکمهٔ منو** هم برقرار است (۷٫۶۵) ══
     ۷٫۶۴ سقف را روی `nightLeft_()` بست — و آن تابع اگر ساعتِ شب روشن
     نباشد همان لحظه روشنش می‌کند. از دکمهٔ منو کسی ساعت را روشن نکرده،
     پس نخستین پرسش (که سرِ خودِ دنباله است) جواب می‌داد «۲۷۰ ثانیه
     مانده» در حالی که ۳۰۰ ثانیه خرج شده بود. صاحبِ برنامه دکمه را زد و
     `Exceeded maximum execution time` گرفت.
     ادعا: وقتی بلوک شروع می‌شود، ساعت باید **از قبل** راه افتاده باشد —
     از داخلِ خودِ مرحله سنجیده می‌شود، نه با خواندنِ ترتیبِ متن. */
  {
    global._nightT0 = 0;                        // همان حالتِ اجرای دستی
    let clockAtBuild = 0;
    const realRun = global.embRunDue_;
    global.embRunDue_ = function () {
      clockAtBuild = Number(global._nightT0) || 0;
      return realRun.apply(null, arguments);
    };
    embNightly_({ cap: 3, budgetMs: 30000 });
    global.embRunDue_ = realRun;
    ok('۲۰.۴-ب از مسیرِ دستی هم ساعت پیش از کار روشن است',
       clockAtBuild > 0,
       clockAtBuild ? 'ساعت سرِ ورود روشن شد' :
                      'ساعت تا سرِ دنباله روشن نمی‌شود ⇒ سقف بی‌اثر است');
    nightStart_();
  }
