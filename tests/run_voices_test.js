...
console.log('\n=== ۹. ردِ قالبِ دستورِ لحن ===');
{
  ...
  /* ══ و حکم باید به جایی برسد که انتخابِ مدل می‌خوانَدش (۷٫۴۷) ══
     تا ۷٫۴۶ حکم فقط در یک رشته می‌نشست و هر روز گزارش می‌شد، ولی
     `resolveModels_` هیچ‌وقت نگاهش نمی‌کرد: همان مدل دوباره انتخاب
     می‌شد، تا ابد. و این از **مسیرِ واقعی** سنجیده می‌شود، نه با
     نوشتنِ نقشه به دست — حالتی که خودِ موتور نسازد، چیزی را ثابت
     نمی‌کند (۷٫۲۲). */
  ok('۹.۸-ب و در نقشه‌ای نشست که انتخابِ مدل می‌خوانَدش',
     ttsCueBadNow_(String(global.__PROPS[PK.TTS_CUE_OFF])) === true,
     'گرفت: ' + JSON.stringify(ttsCueBadMap_()));
  ok('۹.۸-پ ولی مدلِ دیگری را بی‌گناه اعلام نمی‌کند',
     ttsCueBadNow_('gemini-9.9-flash-tts-preview') === false,
     'نقشه‌ای که همه را بد بداند، هیچ جایگزینی باقی نمی‌گذارد');
