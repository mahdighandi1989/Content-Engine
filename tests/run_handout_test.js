...
console.log('\n=== ۸) نقشهٔ راه نباید کتابِ زیرِ خودش را تکذیب کند (۶٫۹۵) ===');
{
  /* دادهٔ واقعیِ جزوهٔ «Audi» در ۶ سپتامبر: پیشرفت «۱۳٫۰۲ از ۸۶ (۱۵٪)» بود،
     پس هر سه مرحله از روی مکان‌نما «۱ در جریان، ۲ و ۳ پیشِ رو» درمی‌آمد —
     در حالی که فصلِ ۶ و فصلِ ۸ همان کتاب دقیقاً عنوانِ مرحله‌های ۲ و ۳ را
     داشتند و کامل نوشته شده بودند. عنوان‌ها اینجا عیناً از همان فایل‌اند. */
  const book = {
    roadmap: { stages: [
      { title: 'مفاهیم پایه، ارزش معرفت و منابع شناخت' },
      { title: 'ادراک حسی، اقتصاد شناختی و مراتب توجیه' },
      { title: 'نظریه‌های ادراک، مسئلهٔ توهم و داده‌های حسی' }
    ] },
    chapters: [
      { title: 'درآمدی بر نظریه شناخت و مسائل بنیادین معرفت‌شناسی' },
      { title: 'اقتصاد شناختی، مراتب ادراک و سرشت توجیه موقعیتی' },
      { title: 'نظریه‌های ادراک، مسئلهٔ توهم و داده‌های حسی' }
    ]
  };
  handoutRoadmapState_(book, { done: 13.02, total: 86 });
  const st = book.roadmap.stages.map((x) => x.state);
  ok('۸.۱ مرحله‌ای که کتاب برایش فصل دارد، «پیشِ رو» نیست',
     st[1] !== 'پیشِ رو' && st[2] !== 'پیشِ رو', st.join(' / '));
  ok('۸.۲ ولی «انجام‌شده» هم ادعا نمی‌شود — وجودِ فصل یعنی شروع، نه پایان',
     st.indexOf('انجام‌شده') === -1, st.join(' / '));
  ok('۸.۳ و عددِ پیشرفت دست‌نخورده می‌مانَد',
     book.roadmap.progress.pct === '15', JSON.stringify(book.roadmap.progress));

  /* و مرحله‌ای که هیچ فصلی ندارد همچنان «پیشِ رو» است — وگرنه سدِ تازه
     نقشهٔ راه را یکسره بی‌معنا می‌کرد. */
  const b2 = {
    roadmap: { stages: [{ title: 'مفاهیم پایه، ارزش معرفت و منابع شناخت' },
                        { title: 'زبان، معنا و دلالت در فلسفهٔ تحلیلی' }] },
    chapters: [{ title: 'درآمدی بر ارزش معرفت و منابع شناخت' }]
  };
  handoutRoadmapState_(b2, { done: 1, total: 100 });
  ok('۸.۴ مرحلهٔ بی‌فصل همچنان «پیشِ رو» می‌مانَد',
     b2.roadmap.stages[1].state === 'پیشِ رو',
     b2.roadmap.stages.map((x) => x.state).join(' / '));

  /* ZWNJ با فاصله جایگزین می‌شود، نه با هیچ — وگرنه «معرفت‌شناسی» هرگز با
     «معرفت» جور نمی‌شود. همان درسِ `speakBone_`. */
  ok('۸.۵ نیم‌فاصله واژه‌ها را به هم نمی‌چسبانَد',
     handoutTitleShare_('معرفت‌شناسی و منابع شناخت', 'منابع معرفت') >= 2,
     String(handoutTitleShare_('معرفت‌شناسی و منابع شناخت', 'منابع معرفت')));

  /* ══ نشانه‌گذاریِ عربیِ چسبیده به واژه (7.06) ══
     دادهٔ واقعیِ جزوهٔ «Audi»: فصلِ ۱۱ («معرفت‌شناسی حافظه، کارکرد یادآوری و
     تبیین خطای ادراکی») همان مرحلهٔ «ساختار معرفتی حافظه و یادآوری» را
     پوشش می‌دهد، ولی ویرگولِ عربیِ بی‌فاصله («حافظه،») که در بازهٔ
     ۰۶۰۰-۰۶FF جا گرفته «حافظه» را از تشخیص می‌انداخت — فقط «یادآوری» مشترک
     می‌ماند، یک واژه، زیرِ سقفِ پیش‌فرضِ ۲. */
  ok('۸.۷ ویرگولِ عربیِ چسبیده واژه را از تشخیص نمی‌اندازد',
     handoutTitleShare_('ساختار معرفتی حافظه و یادآوری',
       'معرفت‌شناسی حافظه، کارکرد یادآوری و تبیین خطای ادراکی') >= 2,
     String(handoutTitleShare_('ساختار معرفتی حافظه و یادآوری',
       'معرفت‌شناسی حافظه، کارکرد یادآوری و تبیین خطای ادراکی')));

  /* ══ سه‌حرفیِ معنادار، که تا ۷٫۴۸ کنار می‌رفت (۷٫۴۹) ══
     دادهٔ واقعیِ جزوهٔ «Audi»: مرحلهٔ «عقل، بداهت و معرفت پیشینی» «پیشِ رو»
     می‌ماند در حالی که فصلِ ۱۳ («عقل، بداهت و گزاره‌های تحلیلی») دقیقاً
     همین را کامل نوشته — تنها با اضافه‌شدنِ «عقل» (سه‌حرفی) به «بداهت»
     سهم به ۲ می‌رسد. */
  ok('۸.۸ واژهٔ سه‌حرفیِ معنادار («عقل») هم به سهم اضافه می‌شود',
     handoutTitleShare_('عقل، بداهت و معرفت پیشینی',
       'عقل، بداهت و گزاره‌های تحلیلی') >= 2,
     String(handoutTitleShare_('عقل، بداهت و معرفت پیشینی',
       'عقل، بداهت و گزاره‌های تحلیلی')));

  /* و مرزش: یک واژهٔ سه‌حرفیِ مشترکِ تنها (بی هیچ ۴+حرفیِ دیگر) هنوز به
     تنهایی زیرِ سقفِ پیش‌فرض می‌مانَد — باز شدنِ سه‌حرفی‌ها به‌معنای حذفِ
     نیازِ «دستِ‌کم ۲ واژه» نیست. */
  ok('۸.۹ یک واژهٔ سه‌حرفیِ مشترکِ تنها هنوز مرحله را پوشیده نمی‌کند',
     handoutTitleShare_('علم و دین در گفتگوی تاریخی',
       'دین و اخلاق در جامعهٔ مدرن') < 2,
     String(handoutTitleShare_('علم و دین در گفتگوی تاریخی',
       'دین و اخلاق در جامعهٔ مدرن')));

  /* و ایست‌واژه‌های سه‌حرفی همچنان بیرون می‌مانند — باز شدنِ سقف برای
     سه‌حرفی‌ها نباید حرفِ ربط را هم قاچاقی وارد کند. */
  ok('۸.۱۰ ایست‌واژه‌های سه‌حرفی («این»، «های»، «که») همچنان کنار می‌روند',
     !handoutTitleWords_('این که های آن را')['این'] &&
     !handoutTitleWords_('این که های آن را')['های'] &&
     !handoutTitleWords_('این که های آن را')['که']);

  /* و تغییرِ *وضعیت* باید باعثِ بازسازیِ HTML شود، نه فقط تغییرِ عدد: تا
     ۶٫۹۵ فقط `progress` مقایسه می‌شد، پس این اصلاح در JSON می‌نشست و در
     خودِ جزوه دیده نمی‌شد. */
  const sigA = handoutRoadmapSig_(book);
  book.roadmap.stages[1].state = 'پیشِ رو';
  ok('۸.۶ امضای نقشهٔ راه وضعیتِ مرحله‌ها را هم می‌بیند',
