 ...
 */
{
  ...
  /* ══ نگهبان باید **همهٔ** پنجره‌ها را ببیند (۷٫۲۳) ══
     تا ۷٫۲۲ فقط تختهٔ مجموعه‌ها پیمایش می‌شد. پنجرهٔ تازهٔ جست‌وجو دکمه‌های
  ...
  ok('۵.۱ پنجره‌ها واقعاً دکمهٔ سروری دارند', called.length >= 5, called.join(', '));
  ok('۵.۱-ب دکمهٔ جست‌وجو هم دیده شد', called.indexOf('srchRun') !== -1,
     'اگر این نبود یعنی پنجرهٔ تازه از دیدِ نگهبان پنهان مانده');
  ...
  /* ══ ۵.۲-ب — و خودِ اسکریپتِ پنجره باید **پارس شود** (۷٫۴۳) ══
     ۵.۲ می‌پرسد «تابعی که صدا زده می‌شود وجود دارد؟» و جوابش برای هر دو
     پنجره **بله** بود، در حالی که هیچ‌کدام کار نمی‌کردند.

     علت یک لایه پایین‌تر بود: رشته‌های سازندهٔ پنجره تک‌کوتیشن‌اند، پس
     `\"` پیش از رسیدن به مرورگر به `"` فرو می‌ریزد. نتیجه نقل‌قولِ نابسته
     است، کلِ بلوکِ <script> پارس نمی‌شود، و **هیچ** تابعی در آن تعریف
     نمی‌شود — نه `go`، نه `esc`، نه `save`. هر دکمه بی‌صدا هیچ می‌کند:
     همان بدترین شکلِ خرابی که این مخزن اسمش را برده و برایش ۵.۲ را
     ساخته بود.

     پس این سنجه یک لایه پایین‌تر می‌ایستد: HTML را رندر کن، هر بلوکِ
     <script> را بیرون بکش، و با `new Function` پارسش کن. اجرا نمی‌شود
     (به DOM و `google` نیاز دارد) — فقط نحو سنجیده می‌شود، که دقیقاً
     همان چیزی است که شکسته بود. */
  {
    const bad = [];
    for (const fn of BUILDERS) {
      let h = '';
      try { h = String(global[fn]()); } catch (e) { continue; }
      const blocks = h.match(/<script>[\s\S]*?<\/script>/g) || [];
      if (!blocks.length) continue;
      blocks.forEach((b, i) => {
        const code = b.replace(/^<script>/, '').replace(/<\/script>$/, '');
        try { new Function(code); }
        catch (e) { bad.push(fn + '#' + i + ': ' + e.message); }
      });
    }
    ok('۵.۲-ب اسکریپتِ هر پنجره پارس می‌شود', bad.length === 0,
       bad.length ? bad.join(' | ')
                  : 'وگرنه هیچ تابعی تعریف نمی‌شود و هر دکمه بی‌صدا هیچ می‌کند');
  }
