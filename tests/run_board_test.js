...
const FILES = ['00_Config.gs','01_Taxonomy.gs','02_Sync.gs','03_Producer.gs','04_Mailer.gs',
               ...
               '14_Special.gs','15_Board.gs','16_Curate.gs','17_Backup.gs','18_Files.gs','19_Enrich.gs','20_Voices.gs','21_SelfUpdate.gs','22_SourceScripts.gs','23_Music.gs','24_ContentAudit.gs','25_Calendar.gs','26_Handout.gs','27_YouTube.gs','28_SourceQuality.gs','29_Explain.gs','30_Recap.gs','31_Bridge.gs','32_Persona.gs', '33_VoiceIntake.gs'];
...
console.log('\n=== ۱۸) ظاهرِ تخته و رسیدِ ماندگارِ هر کار ===');
{
  ...
  ok('۱۸.۱-ب هر جعبهٔ لغزان بسته هم شده است',
     ...
     'باز و بسته نابرابر');
  /* ══ اشتباهِ خودِ ۶٫۵۱، که ظاهر را بدتر کرد (۶٫۵۲) ══
     `overflow-wrap:anywhere` روی همهٔ خانه‌ها، همراهِ `width:100%`، به
     مرورگر می‌گوید کمینهٔ عرضِ هر ستون یک نویسه است. یازده ستون تا یک حرف
     فشرده شدند و «Epistemology» عمودی، حرف‌به‌حرف، زیرِ هم چیده شد.
     جدولِ داخلِ جعبهٔ لغزان **کف عرض** می‌خواهد، نه اجازهٔ شکستن. */
  ok('۱۸.۱-پ هیچ خانه‌ای اجازهٔ شکستنِ حرف‌به‌حرف ندارد',
     h.indexOf('overflow-wrap:anywhere') === -1 &&
     h.indexOf('word-break:break-word') === -1,
     'اجازهٔ شکستن هرجا برگشته');
  ok('۱۸.۱-ت جدول کفِ عرض دارد و فقط ستونِ نام واژهٔ بلند را می‌شکند',
     h.indexOf('.tw>table{min-width:') !== -1 &&
     /\.nm\{[^}]*overflow-wrap:break-word/.test(h) &&
     h.indexOf('class="nm"') !== -1);
  /* و ستونِ دکمه‌ها آخرین ستون است: در راست‌به‌چپ، اولین چیزی که با تنگ‌شدنِ
     پنجره از دید بیرون می‌رود. باید بچسبد، وگرنه در نمایشگرِ کوچک اصلاً
     نمی‌شود روی مجموعه کاری کرد. */
  ok('۱۸.۱-ث ستونِ دکمه‌ها با لغزشِ افقی از دید بیرون نمی‌رود',
     /\.act\{[^}]*position:sticky/.test(h) && h.indexOf('class="act"') !== -1);
