...
console.log('=== ۳) پیش‌فرض سخت‌گیر است: فقط اعراب ===');
{
  /* ══ چرا پیش‌فرض عوض شد (۶٫۲۶) ══
   * ۶٫۲۰ به مدل اجازه داد ویرگول و نیم‌فاصله بیفزاید تا عبارت‌بندی بهتر
   * شود. نتیجه در صوتِ واقعی وارونه بود: ویرگولِ نابه‌جا مکثِ نابه‌جا
   * می‌سازد و لحن را از آنچه بود بدتر می‌کند. «اجازهٔ بیشتر به مدل» با
   * «نتیجهٔ بهتر» یکی نیست، و تنها سنجهٔ درستش گوشِ صاحبِ برنامه بود.
   * پس پیش‌فرض برگشت به همان چیزِ شناخته‌شده: **فقط اعراب**. */
  ok('۳.۰ پیش‌فرضِ نشانه‌گذاری خاموش است', CFG.SPEAK_MARKS === false);
  ok('۳.۱ پس ویرگولِ افزوده هم رد می‌شود', !verifySpeak_(PLAIN, GOOD));
  ok('۳.۲ و اعرابِ تنها پذیرفته می‌شود', verifySpeak_(PLAIN, VBAD));
  ...
  ok('۳.۶ و نیم‌فاصلهٔ افزوده هم نه — حتی اگر به‌نظر درست بیاید',
     !verifySpeak_(PLAIN, ZWNJ));

  /* و اگر کسی روزی دوباره روشنش کرد، باید بداند دقیقاً چه چیزی باز می‌شود:
     ویرگول آزاد می‌شود، ولی نیم‌فاصله هرگز. */
  const keep = CFG.SPEAK_MARKS;
  CFG.SPEAK_MARKS = true;
  ok('۳.۷ با روشن‌کردن، ویرگول آزاد می‌شود', verifySpeak_(PLAIN, GOOD));
  ok('۳.۸ ولی نیم‌فاصله در هیچ حالتی آزاد نیست',
     !verifySpeak_(PLAIN, ZWNJ));
