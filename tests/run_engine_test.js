...
let dg = 0; while (global.__PROPS['PENDING_EPISODE'] && dg++ < 60) { const x = produceEpisodeContinue(); if (x) r = x; }
...
// markUsed_ عمداً همهٔ ردیف‌های هم‌شناسه را علامت می‌زند، پس ≥ تعداد آیتم‌هاست
log('  آیتم‌های علامت‌خورده:', used, used >= CFG.ITEMS_PER_EPISODE ? '✅ (شامل ردیف‌های تکراری همان فایل‌ها)' : '❌');
if (used < CFG.ITEMS_PER_EPISODE) throw new Error('mark-used failed');
...
let r2 = produceEpisode();
let dg2 = 0; while (global.__PROPS['PENDING_EPISODE'] && dg2++ < 60) { const y = produceEpisodeContinue(); if (y) r2 = y; }
