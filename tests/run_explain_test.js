...
console.log('\n=== ۱۳) سدِ ساکت: «بعضی روزا هست بعضی روزا نیست» (۶٫۹۷) ===');
{
  /* گزارشِ صاحبِ برنامه، ۷ سپتامبر. قسمتِ ۳۲ صفر توضیح‌دهنده گرفت با خطِ
     «هیچ پیشنهادی از سدها رد نشد» — جمله‌ای که نمی‌گوید چرا. علت قرعه
     نبود: کف ۹۰۰ نویسه است و مدل‌ها کف را کم می‌زنند. */
  const drop = { got: 3, short: 3, bad: 0, dup: 0, longest: 412 };
  const why = explainDropWhy_(drop, 900);
  ok('۱۳.۱ علت با عدد گفته می‌شود، نه «از سدها رد نشد»',
     /۳|3/.test(why) && /900/.test(why) && /412/.test(why), why);
  ok('۱۳.۲ و شمارهٔ بخشِ ناموجود از کوتاهی جدا شمرده می‌شود',
     /شمارهٔ بخشِ ناموجود/.test(explainDropWhy_(
       { got: 2, short: 0, bad: 2, dup: 0, longest: 0 }, 900)));
  ok('۱۳.۳ نبودِ پیشنهاد با ردشدنِ پیشنهاد یکی نیست',
     explainDropWhy_({ got: 0 }, 900) === 'مدل جایی پیشنهاد نداد');

  /* و گزینش خودش شمارش می‌کند — بی این، هر رفعِ بعدی باید کد را بخواند. */
  const secs = [{ heading: 'یک' }, { heading: 'دو' }];
  const spots = [], used = {}, d2 = { bad: 0, dup: 0, short: 0, longest: 0, got: 0 };
  explainPick_({ spots: [
    { section: '9', at: 'ابتدا', text: 'x'.repeat(2000) },   // بخشِ ناموجود
    { section: '1', at: 'ابتدا', text: 'کوتاه' },            // زیرِ کف
    { section: '1', at: 'انتها', text: 'y'.repeat(1200) }    // سالم
  ] }, secs, 3, 900, 100000, spots, used, d2);
  ok('۱۳.۴ جای سالم می‌مانَد', spots.length === 1 && spots[0].section === 1);
  ok('۱۳.۵ و هر علتِ افتادن جدا شمرده می‌شود',
     d2.bad === 1 && d2.short === 1 && d2.longest === 1200, JSON.stringify(d2));

  /* پرسشِ دوباره فقط وقتی است که **همه** کوتاه بوده باشند: توهمِ شماره با
     پرسیدنِ دوباره درست نمی‌شود و یک فراخوانِ مدل مفت خرج می‌کند. */
  const src29 = fs.readFileSync('src/29_Explain.gs', 'utf8');
  ok('۱۳.۶ پرسشِ دوباره مشروط است، نه همیشگی',
     /drop\.short !== r\.spots\.length/.test(src29));
  ok('۱۳.۷ و کمبود با عدد به مدل گفته می‌شود',
     /کوتاه‌تر از کفِ/.test(src29) && /بلندترینش/.test(src29));
}
