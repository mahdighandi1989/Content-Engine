...
const FILES = ['00_Config.gs','01_Taxonomy.gs','02_Sync.gs','03_Producer.gs','04_Mailer.gs',
  ...
  '26_Handout.gs','27_YouTube.gs','28_SourceQuality.gs','29_Explain.gs','30_Recap.gs','31_Bridge.gs','32_Persona.gs', '33_VoiceIntake.gs', '34_Search.gs', '35_Embed.gs', '36_VoiceBridge.gs'];
...
console.log('=== ۲) ورودی: جزوهٔ همان مجموعه، نه هفده پوشه ===');
{
  ...
  /* ══ ۶٫۸۲ — سقف تنه را کوتاه می‌کند، فصل را نمی‌اندازد ══
     قاعده تا امروز در کامنتِ همین تست نوشته بود («سقف باید همه‌جا ببُرد، نه
     فقط ته») ولی کد «هرچه اول آمد» بود: روی جزوهٔ واقعیِ معرفت‌شناسی
     (۱۵ فصل، ۷۲ بخش) با سقفِ ۱۲٬۰۰۰ متن سرِ فصلِ هفتم قطع می‌شد — و همان
     ۱۲٬۰۰۰ ورودیِ کشفِ ارجاعِ مرورِ بزرگ است. پس نیمهٔ دومِ مجموعه هرگز
     نمی‌توانست ارجاعی بگیرد.
     سنجه هم عوض شد: «طول کم است» را کد قبلی هم پاس می‌کرد. آنچه باید
     ثابت شود این است که **هیچ فصلی غایب نیست** — و مخصوصاً آخری، چون
     همان است که همیشه اول قربانی می‌شد. */
  const big = { chapters: [] };
  for (let i = 1; i <= 12; i++) {
    big.chapters.push({ id: 'C' + i, title: 'فصلِ شمارهٔ ' + i, sections: [
      { id: 'S' + i, title: 'بخشِ شمارهٔ ' + i, body: ('متنی طولانی. '.repeat(60)) }] });
  }
  const cut = recapBookText_(big, 2000);
  let allThere = true;
  for (let i = 1; i <= 12; i++) {
    if (cut.indexOf('فصلِ شمارهٔ ' + i) === -1) allThere = false;
    if (cut.indexOf('بخشِ شمارهٔ ' + i) === -1) allThere = false;
  }
  ok('۲.۳ هیچ فصلی حذف نمی‌شود — سهمِ تنه کم می‌شود، نه شمارِ فصل‌ها',
     allThere && cut.indexOf('جا نشد') === -1, String(cut.length));
  ok('۲.۳-ب و آخرین فصل — که همیشه اولین قربانیِ بریدنِ ته بود — هست',
     cut.indexOf('فصلِ شمارهٔ 12') !== -1 && cut.indexOf('بخشِ شمارهٔ 12') !== -1);
  /* و سهمِ تنه‌ها باید *برابر* باشد، نه «هرچه اول آمد». */
  const seg = cut.split('\n').filter((x) => x.indexOf('•') === 0);
  const lens = seg.map((x) => x.length);
  ok('۲.۳-پ سهمِ برابر: بلندترین و کوتاه‌ترین بخش تقریباً هم‌اندازه‌اند',
     seg.length === 12 && (Math.max.apply(null, lens) - Math.min.apply(null, lens)) < 8,
     seg.length + ' | ' + Math.max.apply(null, lens) + '-' + Math.min.apply(null, lens));
...
console.log('\n=== ۱۹) عکسِ محتوای درس‌نامه بی‌شرطِ خراب ===');
{
  /* ══ خطری که ۶٫۳۰ خودش ساخت، و اصلاحی که خودش باگِ تازه شد (۷٫۰۵) ══
   * مرور از قطعهٔ خام نوشته نمی‌شود، پس اگر به auditSnap_ می‌رسید اِسنادش
   * صفر درصد می‌شد و شمارندهٔ مشترکِ `audit-attrib-low` را بی‌جهت بالا
   * می‌برد. جوابِ ۶٫۳۰ یک `if (meta && meta.recap)` در همین‌جا بود — ولی
   * `produceSpecialEpisode` هرگز `meta` تعریف نمی‌کند؛ مرور از تابعِ کاملاً
   * جداگانهٔ `runRecapEpisode` (بخشِ ۳۰) نوشته می‌شود که مستقیم
   * `writeSpecialJson_` را صدا می‌زند و هرگز به این تابع نمی‌رسد. یعنی آن
   * شرط برای قسمتِ عادی هم `ReferenceError: meta is not defined` می‌داد،
   * try/catchِ زیرش می‌بلعیدش، و auditSnap_ برای *هیچ* قسمتِ درس‌نامه‌ای
   * اجرا نمی‌شد — نه فقط برای مرورها. شاهد: آخرین `_AUDIT-special-*.json`
   * در درایو مالِ قسمتِ ۱۸ بود در حالی که تولید تا ۴۰ رفته بود.
   * پس این تست دیگر دنبالِ آن شرط نمی‌گردد؛ دنبالِ نبودنش می‌گردد. */
  const src = fs.readFileSync('src/14_Special.gs', 'utf8');
  const i = src.indexOf('auditSnap_(ENRICH_SHOW_SPECIAL');
  ok('۱۹.۱ فراخوانِ عکسِ محتوا در produceSpecialEpisode هست', i > 0);
  ok('۱۹.۲ و دیگر پشتِ ارجاعی به «meta» (که در این تابع تعریف نمی‌شود) نیست',
     src.lastIndexOf('meta && meta.recap', i === -1 ? 0 : i) === -1);
  ok('۱۹.۳ علتِ نبودنِ شرط در کامنت مستند است، نه در سکوت',
     src.indexOf('produceSpecialEpisode`) نمی‌رسد') !== -1 ||
     src.indexOf('هرگز `meta` تعریف نمی‌کند') !== -1 ||
     src.indexOf('در این تابع اصلاً تعریف نشده') !== -1);
}
...
console.log('=== ۲۶) قرینهٔ ۶٫۸۳ در مرور (۶٫۸۳) ===');
{
  /* در قسمتِ ۲۷ بلوکِ ارجاع در پرامپت بود و نویسنده نادیده‌اش گرفت. چارهٔ
     درس یک خط در «ساختار خروجی» بود؛ مرور همان شکل را دارد، پس همان خط
     اینجا هم لازم است — «قرینه‌ای که یک بار درست شود، یک بار درست شده است». */
  const pr = recapPrompt_(BOOK, 'معرفت‌شناسی', 9000, 'همه',
                          bridgeRecapBlock_([{ ep: '3', refSeries: 'خداشناسی',
                            kind: 'پیش‌نیاز', at: 'الف', claim: 'ادعا',
                            relation: 'نسبت' }]));
  ok('۲۶.۱ بلوکِ ارجاع در پرامپتِ مرور هست', pr.indexOf('خداشناسی') !== -1);
  ok('۲۶.۲ و کنارش دستورِ صریحِ «باید در متنِ یکی از بخش‌ها بیاید»',
     pr.indexOf('اختیاری نیستند') !== -1 && pr.indexOf('نامِ صریحِ آن مجموعه') !== -1);
  ok('۲۶.۳ مروری بی ارجاع، آن دستور را هم ندارد',
     recapPrompt_(BOOK, 'معرفت‌شناسی', 9000, 'همه', '').indexOf('اختیاری نیستند') === -1);
}

console.log('=== ۲۵) نیمهٔ ۶٫۸۱ که باید به مرور هم می‌رسید (۶٫۸۲) ===');
{
  /* سؤالِ صاحبِ برنامه: «الان که باگ ارجاع دیدی، توی قسمت مرور بزرگ که
     می‌شه ارجاع داد هم ببین آیا این باگ برطرف شده؟»
     نیمی از آن رایگان درست بود: مرور همان `bridgeFor_` را صدا می‌زند، پس
     پیش‌آهنگ و خواندنِ ژرفِ کتابِ *مرجع* را به ارث برد. نیمهٔ دیگر نه —
     خلاصهٔ **خودِ مجموعه** از `recapBookText_` می‌آمد و آن هنوز از سرِ
     متن می‌بُرید. */
  const src = fs.readFileSync('src/30_Recap.gs', 'utf8');
  ok('۲۵.۱ ورودیِ کشفِ ارجاعِ مرور از همان تابعِ کتاب می‌آید',
     /digest: recapBookText_\(book, 12000\)/.test(src));
  ok('۲۵.۲ و آن تابع دیگر «هرچه اول آمد» نیست — سهمِ برابر می‌دهد',
     /Math\.floor\(\(lim - fixed\) \/ nSec\)/.test(src) &&
     src.indexOf("L.push('… (ادامهٔ کتاب جا نشد)')") === -1);
  /* و سنجهٔ رفتاری، نه متنی: کتابی به اندازهٔ جزوهٔ واقعی، با همان سقفی که
     مرور واقعاً می‌فرستد. پیش از ۶٫۸۲ فصلِ هفتم به بعد اصلاً نمی‌رسید. */
  const real = { seriesKey: 'kR', seriesName: 'ر', chapters: [] };
  for (let i = 1; i <= 15; i++) {
    const secs = [];
    for (let j = 1; j <= 5; j++) {
      secs.push({ id: 'S' + i + '_' + j, title: 'بخشِ ' + i + '-' + j,
                  body: 'جمله‌ای از متنِ درس. '.repeat(32) });
    }
    real.chapters.push({ id: 'C' + i, title: 'فصلِ ' + i, sections: secs });
  }
  const dg = recapBookText_(real, 12000);
  let seen = 0;
  for (let i = 1; i <= 15; i++) if (dg.indexOf('فصلِ ' + i) !== -1) seen++;
  ok('۲۵.۳ هر پانزده فصل به کشفِ ارجاعِ مرور می‌رسند، نه هفت‌تای اول',
     seen === 15, seen + ' فصل از ۱۵');
  ok('۲۵.۴ و همهٔ ۷۵ بخش هم نام برده می‌شوند',
     (function () {
       for (let i = 1; i <= 15; i++) for (let j = 1; j <= 5; j++) {
         if (dg.indexOf('بخشِ ' + i + '-' + j) === -1) return false;
       }
       return true;
     })());
  /* و همان تابع ورودیِ خودِ نویسندهٔ مرور است، جایی که `recapChecklist_`
     نامِ هر فصل را می‌دهد و می‌گوید «هیچ‌کدام نباید غایب باشد». مدل نباید
     مأمورِ مرورِ فصلی شود که متنش را ندیده. */
  const wr = recapBookText_(real, 42000);
  let seen2 = 0;
  for (let i = 1; i <= 15; i++) if (wr.indexOf('فصلِ ' + i) !== -1) seen2++;
  const chk = recapChecklist_(real);
  ok('۲۵.۵ سیاههٔ تیک و متنِ فرستاده‌شده یکی‌اند — ادعا و اندازه (۶٫۳۹)',
     seen2 === 15 && chk.indexOf('فصلِ 15') !== -1, seen2 + ' فصل از ۱۵');
}
