/* ============================================================================
 *  موتور محتوا و پادکست — نسخهٔ ۵٫۱۱
 *  (همهٔ بخش‌ها در یک فایل؛ همین را کامل کپی کنید و در Apps Script بچسبانید)
 *
 *  ══ چرا ۵٫۱۱: «شماره و دستهٔ دستی» درست و کامل شد (درخواستِ سومِ شما) ══
 *
...
function selfUpdateStep(force) {
  ...
  if (r.ok) {
    ...
    try {
      ...
      got.file.setContent(JSON.stringify(info, null, 1));
    } catch (eM) {}
  }
  return r;
}
...
function afterCodeSwap() {
  ...
  try {
    ...
    if (got && String(got.info.version) === String(want)) {
      ...
      got.file.setContent(JSON.stringify(got.info, null, 1));
    }
  } catch (e2) {}
