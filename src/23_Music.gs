...
/**
 * آیا اصلاً می‌شود این را **پخش** کرد؟ (۷٫۶۸)
 *
 * ══ جمله‌ای که نوشته شده بود و کد اجرایش نمی‌کرد ══
 * راهنمای این مخزن، دربارهٔ همین بخش، با حروفِ درشت می‌گوید:
 * «**پیش‌فرض ردّ است:** اگر مدل در دسترس نباشد اندازه‌ها می‌مانند و شک
 * یعنی نه. مدلِ غایب، تأییدِ خاموش نیست.»
 *
 * در کد برعکس بود. `musicAccept_` وقتی مدل نشنیده باشد `ok: true` با
 * `heard: ''` برمی‌گرداند، و `heardCanEdge_` فقط «زمینه» را رد می‌کرد —
 * پس ردیفی با داوریِ خالی از هر دو سد رد می‌شد و می‌رفت سرِ **آغازِ**
 * پادکست. مدرکش از خودِ درایو: شناسنامهٔ یک قطعهٔ بانک، ۲۳ سپتامبر،
 * `"heard": ""` کنارِ `"verdict": "مدل نشنید؛ از روی اندازه‌ها: …"`.
 *
 * صاحبِ برنامه این را شنید و کلمه‌اش «آبرو بر» بود: نفس‌کشیدن، صدای
 * موتور و خیابان، سرِ آغاز و پایانِ قسمت‌هایی که منتشر شده‌اند.
 *
 * ══ چرا اینجا و نه در `musicAccept_` ══
 * نگه‌داشتنِ فایل در بانک با پخش‌کردنش دو تصمیم‌اند. اگر ورودی را ببندیم،
 * شبی که مدل در دسترس نیست بانک خالی می‌مانَد — همان هفت هفته‌ای که این
 * پرونده ثبت کرده. پس فایل می‌آید و می‌مانَد، **ولی تا شنیده نشود پخش
 * نمی‌شود**. سکوت از صدای موتور بهتر است؛ و نبودِ فایل از هر دو بدتر.
 *
 * ══ و تنها استثنا آدم است ══
 * قاعدهٔ همیشگیِ این بخش: سلیقهٔ کاربر پاک نمی‌شود. یادداشتی که دستِ آدم
 * خورده باشد (یعنی با «خودکار» شروع نشود) تأیید است — همان‌طور که
 * `heardCanEdge_` از روزِ اول می‌پذیرفت.
 */
function heardPlayable_(cell, note) {
  var t = String(cell || '').trim();
  if (t && t.charAt(0) !== '❓') return true;      // داوری هست
  var nt = String(note || '').trim();
  return !!(nt && nt.indexOf('خودکار') !== 0);     // یا آدم گفته
}
...
function musicPick_(bank, slot, moodWords, wantedId) {
  ...
  for (var i = 0; i < bank.length; i++) {
    ...
    /* ── شنیده‌نشده، پخش نمی‌شود — در هیچ جایگاهی (۷٫۶۸) ──
     * سدِ زیر فقط «زمینه» را از لبه دور می‌کرد و دربارهٔ داوریِ **خالی**
     * ساکت بود. پس قطعه‌ای که هیچ‌کس — نه آدم نه مدل — نشنیده بود،
     * می‌توانست آغازِ برنامه باشد. */
...
function musicStatus_() {
  ...
  /* ══ چند قطعه واقعاً قابلِ پخش است — نه چند قطعه در بانک است (۷٫۶۸) ══
   * تا ۷٫۶۶ فقط `tracks` گزارش می‌شد، یعنی «چند فایلِ سالم». ولی از این
   * نسخه قطعهٔ شنیده‌نشده پخش نمی‌شود، پس «۴۰ قطعه» می‌توانست کنارِ یک
   * برنامهٔ بی‌موسیقی بنشیند و هیچ‌کس نفهمد چرا.
   * و مهم‌تر: این همان عددی است که تا امروز هیچ‌جا نبود. تأییدِ شنیداری
  ...
  try {
    ...
    for (var q = 0; q < bk.length; q++) {
      ...
      if (heardPlayable_(bk[q].heard, bk[q].note)) out.playable++;
      else {
        out.unheard++;
        var sl = String(bk[q].slots || '');
        if (!sl || sl.indexOf('شروع') !== -1 || sl.indexOf('پایان') !== -1) out.unheardEdge++;
      }
    }
...
/**
 * یک جملهٔ فارسیِ آماده، **هر روز**، حتی وقتی همه‌چیز خوب است (۷٫۶۸).
 *
 * قاعدهٔ ۵٫۹۰ این پرونده: «صاحبِ برنامه هیچ شیتی باز نمی‌کند — پس هیچ‌چیز
 * نباید فقط در یک شیت زندگی کند». تأییدِ شنیداریِ قطعه‌ها دقیقاً همان‌جا
 * زندگی می‌کرد، و نتیجه‌اش این شد که ماه‌ها صدای موتور و نفس سرِ آغازِ
 * قسمت‌ها پخش شد و تنها گزارش‌دهنده‌اش گوشِ خودِ او بود.
 */
function musicLine_(st) {
  try {
    var fa = function (n) { try { return faDigitsOut_(String(n)); } catch (e) { return String(n); } };
    if (!st || st.enabled === false) return 'موسیقی: خاموش است.';
    var bits = ['قابلِ پخش: ' + fa(Number(st.playable) || 0)];
    if (Number(st.unheard) > 0) {
      bits.push('شنیده‌نشده و بلااستفاده: ' + fa(st.unheard) +
                (Number(st.unheardEdge) > 0
                  ? ' (' + fa(st.unheardEdge) + ' تایشان برای آغاز/پایان نشانه خورده بودند)' : ''));
    }
...
/**
 * بانکی که قطعهٔ نشنیده دارد و پاک نمی‌شود، **خودش** یافته است (۷٫۶۸).
 *
 * ══ چرا یافته و نه فقط یک سطر در ایمیل ══
 * سطرِ فردا جایش را به سطرِ دیگری می‌دهد؛ یافته نه. و این دقیقاً همان
 * وضعی است که ماه‌ها طول کشید و کسی گزارشش نکرد: بازبینیِ شبانه پشتِ
 * `nightHas_` است و کارِ شبانه چند شب پیاپی زودتر مُرده بود، پس صفِ
 * نشنیده‌ها اصلاً جلو نمی‌رفت — و از بیرون همه‌چیز سالم بود.
 *
 * یک شبِ بد یافته نمی‌سازد: شمارش از **اولین** شبی است که بانک قطعهٔ
 * نشنیده داشت، و به محضِ صفر شدن پاک می‌شود. مُهر را خودِ رویداد
 * می‌نویسد، نه یک شمارندهٔ جدا که هر شب بازنشانی شود (۷٫۲۲).
 */
function musicUnheardCheck_(hub, st) {
  try {
    var n = Number(st && st.unheard) || 0;
    var P = props_();
    if (!n) { try { P.deleteProperty(PK.MUSIC_UNHEARD_AT); } catch (eD) {} return false; }
    var since = String(P.getProperty(PK.MUSIC_UNHEARD_AT) || '');
    if (!since) { try { P.setProperty(PK.MUSIC_UNHEARD_AT, nowStr_()); } catch (eS) {} return false; }
    var t = parseWhen_(since);
    if (isNaN(t)) return false;
    var days = Math.floor((new Date().getTime() - t) / 86400000);
    var lim = Math.max(1, Number(CFG.MUSIC_UNHEARD_DAYS) || 5);
    if (days < lim) return false;
    logSelfFinding_(hub || getHub_(), {
      priority: 'جدی', category: 'موسیقی', key: 'music-unheard',
      title: days + ' روز است ' + n + ' قطعهٔ بانک تأییدِ شنیداری ندارد',
      detail: 'از ۷٫۶۸ قطعهٔ شنیده‌نشده پخش نمی‌شود، پس این ' + n + ' تا ' +
              'بلااستفاده‌اند و جایگاه‌هایی که فقط آن‌ها را داشتند ' +
              'بی‌موسیقی می‌مانند. صفِ بازبینی جلو نمی‌رود.',
      instruction: 'بازبینیِ شبانه پشتِ `nightHas_` است — اول ببین کارِ ' +
                   'شبانه به آن بلوک می‌رسد یا زودتر می‌میرد (`nightAt`/' +
                   '`nightDeath_`). اگر می‌رسد ولی مدل نمی‌شنود، علتش را ' +
                   'در `musicListen_` پیدا کن. تا آن موقع «🔎 بازبینیِ ' +
                   'بانک» از منو همه را یک‌جا می‌شنود. پاسخ را با کلیدِ ' +
                   'همین یافته در manifest بنویس.',
      owner: ROWNER_CODE
    });
    return true;
...
/**
 * یکی‌کردنِ نسخه‌های هم‌نامِ `_MUSIC-FEED.json` در ریشهٔ OUTPUT — بی‌قیدوشرط،
 * نه فقط وقتی `musicFetch_`/`musicSeek_` چیزی برای نوشتن دارند.
 *
 * ══ چرا لازم شد ══
 * تسکِ غنی‌سازی ابزاری برای نوشتنِ درجا ندارد، پس هر بار که آدرسی پیدا
 * می‌کند یک فایلِ تازه با همان نام می‌سازد. `musicFeedRead_` این‌ها را
 * درست می‌خواند (اجتماعِ همه، بی‌گمشدنِ داده) و `putOutJson_` هنگامِ
 * نوشتن خودش تکراری‌ها را به سطل می‌بَرد — ولی این یکی‌شدن فقط وقتی
 * می‌افتد که `musicFetch_`/`musicSeek_` واقعاً چیزی برای نوشتن پیدا کنند.
 * وقتی بانک پر است (`thin` خالی) هر دو زودتر برمی‌گردند بی آنکه بنویسند،
 * پس فایل‌های تکراریِ تسک هرگز جارو نمی‌شوند — همانی که هفته‌ها زیرِ
 * چند کلیدِ متفاوت در «گزارش‌های نظارت» تکرار شد.
 *
 * ══ چرا فقط شبانه کافی نبود (۷٫۰۱) ══
 * تسکِ غنی‌سازی هر ساعت می‌تواند یک فایلِ تازه بسازد — یعنی روزی تا ۲۴
 * بار — و این تابع تا ۷٫۰۰ فقط یک بار در شب صدا می‌شد (از
 * `selfUpdateDaily`). فاصلهٔ نصفه‌شب تا نصفه‌شبِ بعد یعنی هر روز، از
 * ساعتِ اول تا شبِ بعد، احتمالِ دیدنِ ۲ نسخه از این فایل — درست همانی
 * که هر وارسیِ روزانه دوباره در `outLayout.dups` گزارش می‌کرد، با اینکه
 * جاروی شبانه واقعاً کار می‌کرد. حالا `syncCatalog` هم هر ۲ ساعت همین را
 * صدا می‌زند؛ فاصلهٔ دیده‌شدنِ یک نسخهٔ تکراری از ~۲۴ ساعت به ~۲ ساعت
 * می‌آید. خودِ تابع بی‌حالت است (فقط می‌خواند و یک‌جا می‌نویسد)، پس صدا
 * زدنش از دو جا بی‌خطر است.
 */
function musicFeedDedup_() {
  if (CFG.MUSIC_ENABLED === false) return;
  try {
    var files = outFilesByName_(MUSIC_FEED_());
    if (files.length < 2) return;
    var feed = musicFeedRead_();
    putOutJson_(MUSIC_FEED_(), { updatedAt: nowStr_(), items: feed.items });
    logLine_('یکی‌سازیِ شبانهٔ _MUSIC-FEED.json: ' + files.length + ' نسخه → ۱.');
  } catch (e) {}
}
...
function runMusicRecheck() {
  ...
  if (r.notes.length) {
    ...
    /* ══ «هیچ‌کدام رد نشد» با «همه تأیید شدند» یکی نیست (۷٫۷۱) ══
       این جمله تا امروز بی‌قید چاپ می‌شد. و درست همان اجرایی که آن را
       پیدا کرد، مدل در دسترس نبود: صفر قطعه شنیده شد، دو قطعه بی‌داوری
       ماندند، و پنجره بست با «همه‌شان موسیقی‌اند». یعنی همان جملهٔ
       «همه‌چیز خوب است» روی وضعی که اصلاً سنجیده نشده — گران‌ترین جملهٔ
       این پرونده.
       و از ۷٫۶۸ خطرش بیشتر هم هست: قطعهٔ بی‌داوری **پخش نمی‌شود**، پس
       «همه‌شان موسیقی‌اند» کنارِ برنامه‌ای می‌نشیند که موسیقی ندارد. */
    var unsureN = 0;
    try {
      var bk2 = musicBank_();
      for (var u3 = 0; u3 < bk2.length; u3++) {
        if (!heardPlayable_(bk2[u3].heard, bk2[u3].note)) unsureN++;
      }
    } catch (eU) { unsureN = -1; }
    if (unsureN === 0) {
      L.push('', 'همه‌شان موسیقی‌اند.');
    } else if (unsureN > 0) {
      L.push('', 'هیچ‌کدام رد نشد — ولی ' + unsureN + ' قطعه هنوز تأییدِ ' +
             'شنیداری ندارد و تا آن نیاید **پخش نمی‌شود**. اگر مدل در ' +
             'دسترس نبوده، دوباره بزنید؛ وگرنه خودتان گوش بدهید و در ' +
             'ستونِ «یادداشت» بنویسید.');
    } else {
      L.push('', 'هیچ‌کدام رد نشد. (وضعیتِ بانک خوانده نشد.)');
    }
  } else {
