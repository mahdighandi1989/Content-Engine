...
function buildSpecialPrompt_(ctx) {
  ...
  /* ══ قسمتِ ۲۷: بلوکِ ارجاع در پرامپت بود و نویسنده کاملاً نادیده‌اش گرفت ══
     در کلِ متنِ نوشته‌شده نامِ مجموعهٔ مرجع یک بار هم نیامد. بلوک جای درستی
     هم داشت (پیش از «ساختار خروجی»)؛ آنچه نبود، یک خط در همان‌جایی بود که
     مدل بخش‌ها را می‌سازد. تکرارِ خواسته در محلِ اجرا، خواهشِ دوباره نیست —
     جای دیگری است. */
  if (ctx.bridgeBlock) {
    L.push('   **و اگر بالا برای این بخش ارجاعی تعیین شده، آن ارجاع باید در ' +
           'narration همین بخش بیاید** — یک بندِ کامل، با نامِ صریحِ آن مجموعه. ' +
           'بخشی که ارجاع دارد، بی ارجاع تحویل داده نمی‌شود.');
  }
...
function produceSpecialEpisode(opt) {
  ...
  try {
    ...
    try {
      /* ══ اینجا هرگز مرور نیست، پس چکِ recap اینجا جایی نداشت (۷٫۰۵) ══
       * مرورِ بزرگ از `runRecapEpisode` (بخشِ ۳۰) نوشته می‌شود، که خودش
       * مستقیم `writeSpecialJson_` را صدا می‌زند و هرگز به این تابع
       * (`produceSpecialEpisode`) نمی‌رسد — پس یک شرطِ «اگر این قسمت مرور
       * است» در همین‌جا از اول بی‌معنی بود. آنچه اینجا بود بدتر از بی‌معنی:
       * شرطش به متغیرِ «meta» ارجاع می‌داد که در این تابع اصلاً تعریف
       * نشده — نه پارامتر است نه محلی. هر بار که آن شرط ارزیابی می‌شد
       * اجرا می‌شد `ReferenceError: meta is not defined` پرتاب می‌شد، همان
       * try/catchِ زیرش آن را می‌بلعید و لاگ می‌کرد «عکسِ محتوای درس‌نامه
       * گرفته نشد» — و نتیجه این بود که auditSnap_ برای *هیچ* قسمتِ
       * درس‌نامه‌ای اجرا نمی‌شد، نه فقط برای مرورها. شاهدِ درایو: آخرین
       * `_AUDIT-special-*.json` مالِ قسمتِ ۱۸ (۲۷ اوت) بود، در حالی که
       * تولید تا قسمتِ ۴۰ رفته بود — بی هیچ خطای «جدی»ای، چون خودِ
       * try/catch دقیقاً برای همین نوع خطا نوشته شده بود.
       * درسِ تکراریِ همین پرونده: تحلیلی که نوشته و تست شد ولی به مسیرِ
       * اشتباه وصل شد، بی‌صدا هیچ کاری نمی‌کند. */
...
function renderSpecialAudioStep_() {
  ...
  try {
    ...
    /* قسمتِ مرورِ بزرگ درسِ تازه نیست و به جزوه فصلی اضافه نمی‌کند: جزوه
       کتابِ درس‌هاست و مرور خلاصهٔ همان کتاب است. افزودنش به خودش کتاب را دو
       برابر می‌کند بی آنکه چیزی به آن بیفزاید — و بدتر، شمارشِ پوششِ جزوه
       را برای همیشه به‌هم می‌ریزد: قسمتی که هرگز فصلی ندارد، هر شب «عقب»
       شمرده می‌شود و هشدارِ handout-stuck هرگز بسته نمی‌شود. */
    if (meta.recap) {
      logLine_('جزوه: قسمت ' + epNum + ' مرورِ بزرگ است، نه درسِ تازه؛ فصلی اضافه نشد.');
    } else try {
