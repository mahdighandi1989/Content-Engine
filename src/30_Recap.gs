...
function recapPrompt_(book, seriesName, capChars, scope, bridges) {
  ...
  if (bridges) {
    L.push('');
    L.push(bridges);
    /* ══ قرینهٔ ۶٫۸۳ — همان خط، در همان جای اجرا ══
       در قسمتِ ۲۷ بلوکِ ارجاع در پرامپتِ درس بود و نویسنده کاملاً نادیده‌اش
       گرفت؛ چاره یک خط در همان‌جایی بود که مدل بخش‌ها را می‌سازد. مرور همان
       شکلِ پرامپت را دارد، پس همان خط اینجا هم باید باشد — قاعدهٔ ۵٫۹۵:
       «قرینه‌ای که یک بار درست شود، یک بار درست شده است.» */
    L.push('');
    L.push('**و این ارجاع‌ها اختیاری نیستند:** هرکدام باید در متنِ یکی از بخش‌ها');
    L.push('بیاید، با نامِ صریحِ آن مجموعه — نه فهرست‌وار در پایان، نه به اشاره.');
  }
  return L.join('\n');
}
 ...
 */
/**
 * سقفِ متنِ مرورِ بزرگ — هدفِ کامل، بی رزرو (۶٫۶۸).
 *
 * ══ «خیلی کوتاه بود و اصلاً نمی‌تونست همه چیز رو پوشش بده» ══
 * گزارشِ صاحبِ برنامه، و کد تأییدش کرد: recapWrite_ هنوز specialFileCap_
 * می‌گرفت — سقفِ *یک فایل* (~۱۱ دقیقه) — در حالی که درس‌ها از ۶٫۴۳ دو
 * فایل مجازند و کامنتِ خودِ پرامپت می‌گفت «حالا سقف ۱۵ دقیقه است». پانزده
 * فصل در یازده دقیقه یعنی ناچار فهرستِ تلگرافی، هر قدر پرامپت اصرار کند.
 *
 * و برخلافِ درس، مرور رزرو هم نمی‌خواهد: غنی‌سازی و عصری‌سازی در مسیرش
 * نیستند (از فازِ speak تحویل می‌شود). پس هدفِ کاملِ پیکربندی مالِ خودِ
 * متن است.
 */
function recapCap_() {
...
function recapBoardMap_(hub, reg) {
  ...
  try {
    ...
    for (var i = 0; i < (reg.rows || []).length; i++) {
       ...
       *
       * ══ ولی «نامعلوم» هم جوابِ آخر نبود (۶٫۴۱) ══
       * صاحبِ برنامه پرسید: «چرا نوشته آخرین انجام نامعلومه؟ مگه جایی ثبت
       * نشده؟» — و حق داشت که بپرسد: **ثبت شده بود، فقط نه در آن فیلد.**
       * شمارهٔ قسمتِ خودِ مرور ذخیره است، و شماره‌ها سراسری و صعودی‌اند. یک
       * مرورِ پیش از ۶٫۳۹ به‌اجبار *کلِ آن‌چه آن روز وجود داشت* را گفته —
       * دامنه‌ای در کار نبود. پس «تا کدام درس» از همان شماره درمی‌آید:
       * بالاترین درسی که شماره‌اش از شمارهٔ آن مرور کمتر است. این استنتاج
       * است، ولی استنتاجِ قطعی، نه حدس.
       *
       * و بهایش را ۶٫۳۹ داشت می‌پرداخت بی آنکه ببیند: چون `unknown` بود،
       * گزینهٔ «فقط درس‌های پس از مرورِ قبلی» برای تنها مجموعه‌ای که مرور
       * داشت **نمایش داده نمی‌شد** — یعنی همان دامنه‌ای که او از اول خواسته
       * بود، دست‌نیافتنی مانده بود. */
      var uptoFrom = upto ? 'ثبت' : '';
      if (d && !upto && !mode) {
        /* فقط برای پرونده‌های پیش از ۶٫۳۹ (`mode` ندارند): آن مرورها دامنه
           نداشتند، پس به‌اجبار کلِ آن‌چه آن روز بود را گفته‌اند. */
        var ep0 = Number(d.ep) || 0, gu = 0, cnt = 0;
        for (var e1 = 0; e1 < eps.length; e1++) {
          if (ep0 && eps[e1] < ep0) { cnt++; if (eps[e1] > gu) gu = eps[e1]; }
        }
        /* و برآورد در برابرِ عددِ **ثبت‌شده** آزموده می‌شود: اگر شمارِ
           درس‌های زیرِ آن مرز با `parts`ِ ضبط‌شده نخواند، استنتاج غلط است و
           کنار می‌رود. یک برآوردِ آزموده‌نشده همان ادعایی است که این ریپو
           بارها از آن ضربه خورده؛ عددِ ثبت‌شده همیشه بر استنتاج مقدم است. */
        if (gu && (!covered || cnt === covered)) { upto = gu; uptoFrom = 'برآورد'; }
      }
      if (!covered && upto) {
        var c1 = 0;
        for (var e3 = 0; e3 < eps.length; e3++) if (eps[e3] <= upto) c1++;
        covered = c1;
      }
