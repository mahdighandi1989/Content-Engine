...
var BRIDGE_KINDS = {
  ...
  /* ══ سه نسبتی که صاحبِ برنامه با نامِ خودش خواست (۶٫۸۱) ══
     «چه از جهتِ تأییدِ مطالبِ هم، چه از جهتِ نقض یا رد… و تکمله‌ها و متمم‌ها
     و مکمل‌ها.» تا امروز هر سه زیرِ «تنش» و «تکمیل» جمع شده بودند — و
     نسبتی که نامِ خودش را نداشته باشد، مدل کمتر پیدایش می‌کند. */
  'تأیید': 'آن مجموعه همین حرف را از راهی دیگر تأیید می‌کند — دو مسیرِ مستقل به یک نتیجه.',
  'نقض': 'آن مجموعه همین ادعا را رد می‌کند یا مثالِ نقضش را دارد؛ اختلاف را باید صریح گفت.',
  'تکمیل': 'آن مجموعه همین بحث را از جای دیگری کامل می‌کند (تکمله، متمم، مکمل).',
 ...
 *
 * تا امروز `digest` این بود: `allText.slice(0, 12000)`. متنِ خامِ یک درس تا
 * ۴۲٬۰۰۰ نویسه است، پس کاشفِ نسبت عملاً فقط **ربعِ اولِ** درس را می‌دید و
 * دربارهٔ سه‌چهارمِ باقی هیچ نمی‌دانست. خواستهٔ صریح این بود که «در هر قسمت
 * از درسِ اصلی به همهٔ متنِ مرجع مراجعه شود» — و طرفِ دیگرش همین است: همهٔ
 * درس هم باید دیده شود، وگرنه نسبتی که با نیمهٔ دومِ درس دارد، پیدا نمی‌شود.
 *
 * پس به‌جای یک برشِ سرِ متن، چند پنجرهٔ هم‌فاصله از سراسرِ آن. بریدنِ
 * ناگزیر است (پرامپت سقف دارد)، ولی *کجا* بریده شود انتخابی است.
 */
function bridgeDigest_(text, cap) {
  var t = String(text || '').replace(/\s+/g, ' ').trim();
  var lim = Math.max(2000, Number(cap) || 12000);
  if (t.length <= lim) return t;
  var win = Math.max(6, Number(CFG.BRIDGE_DIGEST_WINDOWS) || 8);
  var each = Math.floor(lim / win);
  var step = Math.floor(t.length / win);
  var L = [];
  for (var i = 0; i < win; i++) {
    var from = i * step;
    L.push((i ? '… ' : '') + t.substr(from, each));
  }
  return L.join(' ');
}

/* ══ پیش‌آهنگ: کدام بخش‌های کتابِ مرجع را باید *کامل* خواند؟ (۶٫۸۱) ══
 *
 * ══ گزارشی که این را لازم کرد ══
 * «دارم با آن مرجعی که وصلش کردم مقایسه می‌کنم؛ خیلی جاها می‌توانست ارتباط
 * بدهد ولی نتوانسته تشخیص بدهد و گویا خیلی سطحی بررسی می‌کند… در حالی که
 * خودم که خیلی هم عمیق نگاه نکردم می‌بینم خیلی ارتباط دارند، چه از جهتِ
 * تأییدِ مطالبِ هم، چه از جهتِ نقض یا رد.»
 *
 * و راست می‌گفت — ولی نه به‌خاطرِ پرامپت. پرامپتِ کشف از ۶٫۴۳ صریح می‌گوید
 * «به کلِ آن کتاب نگاه کن» و «نسبت لزوماً هم‌موضوعی نیست». عیب در **ورودی**
 * بود: `bridgeCorpus_` فقط اسکلتِ کتاب را می‌داد — عنوانِ فصل‌ها، عنوانِ
 * بخش‌ها، و یک خط چکیده. یعنی مدل فهرستِ مطالب را می‌دید، نه حرف‌ها را. با
 * فهرستِ مطالب می‌شود فهمید دو کتاب هم‌موضوع‌اند یا نه؛ نمی‌شود فهمید یکی
 * دیگری را **تأیید** می‌کند یا **رد**. آن نسبت‌ها در تنِ متن‌اند.
 *
 * چاره‌اش «همه‌چیز را بفرست» نیست: جزوهٔ یک مجموعهٔ هفده‌درسی صدها هزار
 * نویسه است و در هر قسمت فرستادنش هم گران است هم دقت را پایین می‌آورد.
 * پس دو مرحله، همان الگویی که در این ریپو جواب داده (نوشتن و داوری جدا):
 *   ۱) پیش‌آهنگ اسکلتِ **کلِ** کتاب را می‌بیند — همهٔ فصل‌ها، همهٔ بخش‌ها —
 *      و فقط می‌گوید کدام بخش‌ها ارزشِ خواندنِ کامل دارند.
 *   ۲) کاشف متنِ **کاملِ** همان بخش‌ها را می‌گیرد و نسبت را از روی خودِ
 *      حرف‌ها می‌سنجد، نه از روی عنوان.
 * «کلِ متنِ مرجع دیده می‌شود» همچنان راست است — در مرحلهٔ اول، برای هر
 * بخشِ درس. و این دقیقاً همان چیزی است که خواسته شده بود: هیچ تناظری میان
 * شمارهٔ درسِ این مجموعه و شمارهٔ درسِ مرجع در کار نیست.
 */
var BRIDGE_SCOUT_SCHEMA = {
  type: 'object',
  properties: {
    picks: {
      type: 'array',
      items: {
        ...
        properties: {
          seriesKey: { type: 'string' },   // کدام مجموعهٔ مرجع
          sectionId: { type: 'string' },   // شناسهٔ بخش، از قلابِ [] در فهرست
          why: { type: 'string' }          // در چند واژه: چرا ممکن است مرتبط باشد
        },
        required: ['seriesKey', 'sectionId']
      }
    }
  },
  required: ['picks']
...
function bridgeScoutPrompt_(ctx, corpus) {
  var n = Math.max(2, Number(CFG.BRIDGE_DEEP_MAX) || 10);
  var L = [
    'کارِ تو **کشفِ نسبت نیست** — انتخابِ چیزی است که باید کامل خوانده شود.',
    '',
    'یک درس در حالِ نوشته‌شدن است. فهرستِ کاملِ یک یا چند کتابِ درسیِ دیگر را',
    'هم می‌بینی: هر فصل با بخش‌هایش، و هر بخش با شناسه‌اش داخلِ [] و یک خط چکیده.',
    'بگو کدام بخش‌ها را باید **کامل** خواند تا بشود فهمید نسبتی با این درس',
    'هست یا نه.',
    '',
    ...
    '',
    'متنِ این قسمت:',
    String(ctx.digest || '').slice(0, 9000),
    ''
  ];
  for (var i = 0; i < corpus.length; i++) {
    L.push('── فهرستِ کاملِ مجموعهٔ مرجع — شناسه: ' + corpus[i].key + ' ──');
    L.push('نام: «' + corpus[i].name + '»');
    L.push(corpus[i].text);
    L.push('');
  }
  L.push('── قاعده‌ها ──');
  L.push('۱) **هر بخشِ این درس را جداگانه در نظر بگیر** و برای هر کدام کلِ آن');
  L.push('   کتاب(ها) را بگرد. هیچ تناظری میان شمارهٔ این درس و شمارهٔ درسِ آن');
  L.push('   کتاب وجود ندارد؛ نسبتِ درسِ اولِ این می‌تواند با فصلِ آخرِ آن باشد.');
  L.push('۲) دنبالِ هم‌موضوعی نگرد. این‌ها را هم بردار:');
  L.push('   • بخشی که همین ادعا را از راهِ دیگری **تأیید** می‌کند؛');
  L.push('   • بخشی که همین ادعا را **رد** می‌کند یا مثالِ نقض دارد؛');
  L.push('   • بخشی که ابزار یا پیش‌نیازِ فهمِ این حرف را می‌دهد؛');
  L.push('   • بخشی که همین بحث را از جای دیگر **تکمیل** می‌کند.');
  L.push('   عنوان‌ها معمولاً این‌ها را لو نمی‌دهند؛ به چکیده و به جای آن بخش');
  L.push('   در ساختارِ کتاب نگاه کن و اگر شک داری، برش دار — این مرحله فقط');
  L.push('   انتخابِ خواندنی‌هاست، نه تصمیمِ نهایی.');
  L.push('۳) حداکثر ' + n + ' بخش، از میانِ همهٔ کتاب‌ها روی‌هم. کمتر از این');
  L.push('   اشکالی ندارد، ولی اگر واقعاً چیزی هست خالی برنگردان.');
  L.push('۴) `sectionId` باید **عیناً** همان چیزی باشد که داخلِ [] آمده.');
  return L.join('\n');
}

/** متنِ کاملِ بخش‌های انتخاب‌شده. بی خواندنِ دوبارهٔ درایو. */
function bridgeDeepText_(corpus, picks) {
  var byKey = Object.create(null);
  for (var i = 0; i < (corpus || []).length; i++) byKey[corpus[i].key] = corpus[i];
  var perSec = Math.max(400, Number(CFG.BRIDGE_DEEP_SEC_CHARS) || 1800);
  var cap = Math.max(4000, Number(CFG.BRIDGE_DEEP_CHARS) || 26000);
  var out = [], used = 0, n = 0;
  var want = Object.create(null);
  for (var p = 0; p < (picks || []).length; p++) {
    var k = String((picks[p] || {}).seriesKey || '');
    var sid = String((picks[p] || {}).sectionId || '');
    if (!k || !sid) continue;
    want[k + '\u0000' + sid] = true;
  }
  for (var c = 0; c < (corpus || []).length; c++) {
    var book = corpus[c].book, chs = (book && book.chapters) || [];
    var lines = [];
    for (var ch = 0; ch < chs.length; ch++) {
      var secs = chs[ch].sections || [];
      for (var t = 0; t < secs.length; t++) {
        if (!want[corpus[c].key + '\u0000' + String(secs[t].id || '')]) continue;
        var body = String(secs[t].body || '').replace(/\s+/g, ' ').trim();
        /* «تکمیل از درسِ N» — متنی که بعداً به همین بخش افزوده شده. همان‌قدر
           حرفِ کتاب است و نبودش یعنی نصفِ بخش دیده نشود. */
        var adds = secs[t].adds || [];
        for (var a = 0; a < adds.length; a++) {
          body += ' ' + String(adds[a].text || adds[a] || '').replace(/\s+/g, ' ').trim();
        }
        if (!body) continue;
        var piece = '• فصلِ «' + String(chs[ch].title || '') + '» › بخشِ «' +
                    String(secs[t].title || '') + '»:\n' + body.slice(0, perSec);
        if (used + piece.length > cap) { lines.push('… (بقیه جا نشد)'); ch = chs.length; break; }
        lines.push(piece); used += piece.length; n++;
      }
    }
    if (lines.length) {
      out.push('── متنِ کاملِ بخش‌های مرتبطِ «' + corpus[c].name + '» (شناسه: ' +
               corpus[c].key + ') ──\n' + lines.join('\n\n'));
    }
  }
...
function bridgeScout_(ctx, corpus) {
  var r = null;
  try { r = geminiText_(bridgeScoutPrompt_(ctx, corpus), BRIDGE_SCOUT_SCHEMA, 8192); }
  catch (e) { logLine_('پیش‌آهنگِ ارجاع نتیجه نداد: ' + e.message); return null; }
  if (!r || !(r.picks instanceof Array) || !r.picks.length) return null;
  return r.picks.slice(0, Math.max(2, Number(CFG.BRIDGE_DEEP_MAX) || 10));
}

function bridgePrompt_(ctx, corpus, deep) {
  var L = [
  ...
  for (var i = 0; i < corpus.length; i++) {
    L.push('── فهرستِ کاملِ مجموعهٔ مرجع ' + (i + 1) + ' — شناسه: ' + corpus[i].key + ' ──');
    L.push('نام: «' + corpus[i].name + '»');
    L.push(corpus[i].text);
    L.push('');
  }
  /* متنِ کاملِ بخش‌هایی که پیش‌آهنگ انتخاب کرده (۶٫۸۱). فهرستِ بالا برای
     «کجای کتاب» است و این برای «آنجا دقیقاً چه گفته شده» — و نسبت‌هایی مثل
     تأیید و نقض فقط از دومی درمی‌آیند. */
  if (deep && deep.text) {
    L.push('');
    L.push('══ و حالا متنِ کاملِ بخش‌هایی از همان کتاب‌ها که به این درس نزدیک‌اند ══');
    L.push('(این‌ها را یک مرحلهٔ پیشین از میانِ کلِ کتاب انتخاب کرده است.)');
    L.push('');
    L.push(deep.text);
    L.push('');
  }
  ...
  L.push('');
  L.push('۲-ب) **و برای هر بخشِ این درس جداگانه بگرد.** یک درس چند بخش دارد و');
  L.push('   هر بخش می‌تواند نسبتِ خودش را با جای دیگری از آن کتاب داشته باشد.');
  L.push('   نسبت‌هایی که بیش از همه جا می‌مانند این دوتایند، چون در عنوان‌ها');
  L.push('   پیدا نیستند و باید از تنِ متن درآیند:');
  L.push('   • **تأیید** — آن کتاب همین حرف را از راهی دیگر می‌رسانَد؛');
  L.push('   • **نقض** — آن کتاب همین ادعا را رد می‌کند یا مثالِ نقض دارد.');
  L.push('   اختلاف را پنهان نکن: جایی که دو کتاب یکی نمی‌گویند، خودش');
  L.push('   آموزنده‌ترین ارجاع است. ولی نسبت باید در متن باشد، نه ساخته شود.');
  L.push('');
  ...
  L.push('');
  /* ══ فشارِ پرامپت در همان جهتی بود که او شکایت داشت (۶٫۸۳) ══
     «کمتر بهتر است» یک انگشت روی ترازو بود، دقیقاً به سمتِ کم‌شدن — و مدل
     اطاعت می‌کرد. حالا سقف هست ولی ترجیح نیست: نسبتِ واقعی باید گفته شود،
     نسبتِ ساختگی هرگز. این دو با هم می‌آیند، وگرنه «قوی‌ترش کن» به
     «ارجاعِ بی‌ربط بساز» تبدیل می‌شود. */
  L.push('۶) **در هر بخشِ این درس حداکثر یک ارجاع به هر کتاب** — و روی هم');
  L.push('   حداکثر ' + (Number(CFG.BRIDGE_MAX_LINKS) || 3) + ' ارجاع در کلِ قسمت.');
  L.push('   کم یا زیاد بودنِ شمار، خودش نه خوب است نه بد: **هر نسبتِ واقعی');
  L.push('   را بگو، و هیچ نسبتِ ساختگی‌ای نساز.** اگر سه بخش از این درس هر');
  L.push('   سه نسبتِ واقعی با آن کتاب دارند، هر سه را بده؛ اگر فقط یکی دارد،');
  L.push('   فقط همان یکی. ارجاعی که نسبتش «ضعیف» است اصلاً نده — یک قسمتِ');
  L.push('   بی‌ارجاع سالم است، یک قسمت با ارجاعِ ساختگی نیست.');
  L.push('');
...
function bridgePlan_(ctx, corpus) {
  ...
  /* دو مرحله: اول انتخابِ خواندنی‌ها از میانِ کلِ کتاب، بعد کشفِ نسبت از روی
     متنِ کاملِ همان‌ها. اگر پیش‌آهنگ به هر دلیل نتیجه نداد، همان مسیرِ
     پیشین (فقط فهرست) اجرا می‌شود — بدتر از دیروز نمی‌شود. */
  var deep = null;
  if (CFG.BRIDGE_DEEP !== false) {
    try {
      var picks = bridgeScout_(ctx, corpus);
      if (picks) {
        deep = bridgeDeepText_(corpus, picks);
        if (deep && deep.n) {
          logLine_('ارجاع: ' + deep.n + ' بخش از کتاب‌های مرجع کامل خوانده شد.');
        } else deep = null;
      }
...
function bridgeTrim_(links, names, strictKeys) {
  ...
  for (var i = 0; i < (links || []).length && out.length < max; i++) {
    ...
    /* ══ یک ارجاع به هر کتاب در هر **بخش** — نه در هر قسمت (۶٫۸۳) ══
     *
     * تا امروز اینجا نوشته بود `if (seen[key]) continue` با کلیدِ خودِ
     * مجموعه: «یک مجموعه، یک ارجاع در هر قسمت». وقتی فقط یک مجموعهٔ مرجع
     * تیک خورده باشد — که حالتِ واقعی است — این یعنی سقفِ عملیِ هر درس
     * دقیقاً **یک** ارجاع، هرچه هم کشف کرده باشیم. دادهٔ واقعی همین را
     * می‌گوید: شش درسِ «Audi» و شش ارجاع، یک به یک، بی استثنا. پس دو
     * نسخهٔ کارِ روی *ورودیِ* کشف (۶٫۸۱ پیش‌آهنگ و متنِ کامل، ۶٫۸۲ فهرستِ
     * تضمین‌شده) هیچ اثری در خروجی نداشت: هرچه کشف می‌شد، همین‌جا دور
     * ریخته می‌شد جز اولی.
     *
     * خواستهٔ صریحِ صاحبِ برنامه: «در هر قسمت از درسِ اصلی باید به همهٔ
     * متنِ مرجع مراجعه کند برای یافتنِ ارتباط‌ها و تأییدها و ردها و
     * تکمله‌ها و متمم‌ها و مکمل‌ها» — جمع، و به‌ازای هر بخش.
     *
     * ترسِ اصلی («لوث شدن») سرِ جایش است، ولی مرزش عوض می‌شود: دو ارجاع به
     * یک کتاب **در یک بخش** لوث است؛ دو ارجاع در دو بخشِ متفاوتِ یک درس،
     * دو نسبتِ متفاوت‌اند. پس کلیدِ یکتایی «مجموعه + بخش» می‌شود. بخشی که
     * نسبتِ واقعی ندارد همچنان هیچ نمی‌گیرد — رفتنِ سقف بالا اجباری
     * نمی‌آورد. */
    var at = String(x.atHeading || '').trim().toLowerCase().replace(/\s+/g, ' ');
    var dup = key + '§' + at;
 ...
 */
/**
 * متنِ همان بخشی از درس که ارجاع قرار بود آنجا بنشیند — یا '' اگر پیدا نشد.
 *
 * ══ چرا از ۶٫۸۳ لازم شد ══
 * تا وقتی هر کتاب فقط یک ارجاع در هر قسمت داشت، سنجشِ «نامِ مرجع جایی در
 * متن هست؟» کافی بود. حالا که یک کتاب می‌تواند در سه بخشِ متفاوت سه نسبتِ
 * متفاوت داشته باشد، آن سنجه هر سه را با **یک** بار نام‌بردن قبول می‌کرد —
 * یعنی دقیقاً همان دروغی که `bridgeVerify_` برای جلوگیری‌اش نوشته شد، فقط
 * این بار سه‌برابر. پس دامنهٔ سنجش هم باید به همان بخش تنگ شود.
...
function bridgeVerify_(ep, links) {
   ...
   * نمی‌سازد. */
  /* جمله‌های یک متن، آماده برای سنجش — یک بار برای کلِ درس و در صورتِ
     لزوم یک بار برای هر بخش (نتیجه‌ها cache می‌شوند تا سه ارجاعِ یک بخش
     سه بار جمله‌بندی نکنند). */
  var cut = Object.create(null);
  /* بسامد در کلِ قسمت شمرده می‌شود، نه در بخشِ تنگ‌شده: واژه‌ای که در کلِ
     درس پربسامد است، در یک بخشِ کوتاه هم نام‌بَر نمی‌شود. */
  var allN = [];
  var sentsOf = function (txt) {
    var ck = String(txt).length + ':' + String(txt).slice(0, 40);
    if (cut[ck]) return cut[ck];
    var a = [];
    try { a = speakSentences_(txt); } catch (e3) { a = [String(txt)]; }
    if (!a.length) a = [String(txt)];
    var n = [];
    for (var q = 0; q < a.length; q++) n.push(norm(a[q]));
    cut[ck] = { s: a, n: n };
    return cut[ck];
  };
  allN = sentsOf(rawText).n;
  for (var i = 0; i < links.length; i++) {
    ...
    /* دامنه: همان بخشی که ارجاع برایش نوشته شده؛ اگر پیدا نشد، کلِ متن. */
    var scope = '';
    try { scope = bridgeSectionText_(ep, links[i].atHeading); } catch (eSc) { scope = ''; }
    var cs = sentsOf(scope || rawText);
    var sents = cs.s, nsents = cs.n;
    /* ══ واژه‌ای که واژهٔ خودِ درس است، نامِ کتاب نیست (۶٫۸۳) ══
     *
     * سنجهٔ پیشین می‌گفت: اگر **هر کدام** از واژه‌های شاخصِ نامِ مرجع
     * جایی از متن باشد، ارجاع «گفته شد». مرجعِ واقعیِ این سامانه
     * «معرفت شناسی مجتبی مصباح» است و درسِ جاری هم معرفت‌شناسی — پس
     * «معرفت» و «شناسی» واژه‌های روزمرهٔ خودِ متن‌اند. نتیجه در قسمتِ ۲۷
     * دیده شد: سیاهه و پروندهٔ قسمت گفتند ارجاع «گفته شد» و `thin:false`،
     * در حالی که در کلِ ۸٬۴۹۱ نویسهٔ متن واژهٔ «مصباح» **یک بار هم**
     * نیامده بود. دقیقاً همان دروغی که این تابع برای جلوگیری‌اش نوشته شد،
     * از درِ دیگر — و چون دروغ گفت، هشدارِ `bridge-ignored` که همین‌جا
     * آماده بود هرگز بلند نشد.
     *
     * مرز: واژه‌ای نام‌بَر است که در **خودِ این درس کمیاب** باشد. «مصباح»
     * در یک درسِ معرفت‌شناسی صفر بار می‌آید مگر نامِ کتاب گفته شود؛
     * «معرفت» ده‌ها بار. پس بسامدِ هر واژه در جمله‌های همین قسمت شمرده
     * می‌شود و واژه‌های پربسامد از سنجه بیرون می‌روند. آستانه نسبی است نه
     * ثابت: در متنِ بلندتر، «چند بار» معنای دیگری دارد.
     *
     * و اگر **هیچ** واژهٔ کمیابی نماند — یعنی نامِ مرجع تماماً از واژگانِ
     * خودِ درس ساخته شده — سنجه به نردبانِ دوم می‌رود: دستِ‌کم نیمی از
     * واژه‌ها (گِرد به بالا) در یک جمله. سوگیری همچنان محافظه‌کار است:
     * «نمی‌دانم» نباید «نیامده» گزارش شود. */
    var dfMax = Math.max(1, Math.floor(allN.length *
                 (Number(CFG.BRIDGE_TERM_DF_PCT) || 0.03)));
    var rare = [];
    for (var d = 0; d < terms.length; d++) {
      var df = 0;
      for (var dz = 0; dz < allN.length; dz++) {
        if (allN[dz].indexOf(terms[d]) !== -1) df++;
      }
      if (df <= dfMax) rare.push(terms[d]);
    }
    var probe = rare.length ? rare : terms;
    var need = rare.length ? 1 : Math.max(1, Math.ceil(terms.length / 2));
    var hit = false, said = 0;
    for (var sx = 0; sx < nsents.length; sx++) {
      var got = 0;
      for (var k = 0; k < probe.length; k++) {
        if (nsents[sx].indexOf(probe[k]) !== -1) got++;
      }
