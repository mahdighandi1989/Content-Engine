...
function musicHtml_() {
  try {
    ...
    if (last && last.tracks && last.tracks.length) {
      ...
      /* «بانک ۴۰ قطعه دارد» کنارِ یک قسمتِ بی‌موسیقی گمراه‌کننده است، چون
         از ۷٫۶۸ قطعهٔ شنیده‌نشده پخش نمی‌شود. عددی که باید گفته شود
         «چند تا قابلِ پخش‌اند» است، نه «چند فایل سالم است». */
      L.push('این قسمت بی‌موسیقی ساخته شد (' +
             (st.playable == null ? ('بانک ' + st.tracks + ' قطعه دارد')
                                  : (st.playable + ' قطعهٔ قابلِ پخش')) + ').<br>');
    } else {
      L.push('بانکِ موسیقی خالی است؛ قسمت بی‌موسیقی ساخته شد.<br>');
    }
    /* و جملهٔ روزانه، **همیشه** — حتی وقتی همه‌چیز خوب است. تا ۷٫۶۶ عددِ
       «شنیده‌نشده» فقط در تب بود و او شیت باز نمی‌کند (۵٫۹۰). */
    if (st.line && Number(st.unheard) > 0) {
      L.push('<span style="color:#b45309">' + esc_(st.line) + '</span><br>');
    }
