...
function ttsCue_(sectionStyle, text) {
  ...
  // نقطهٔ پایانیِ لحن برداشته می‌شود: کد خودش «. » می‌گذارد و لحنی که به
  // نقطه ختم شود «..» می‌سازد — در رشته‌ای که این مخزن سرِ «مدل خودِ دستور
  // را خواند» تاوانش را داده، آشغالِ نگارشی آشغالِ بی‌هزینه نیست.
  var style = String(sectionStyle || '').replace(/\s+/g, ' ')
                .replace(/[.،؛:]+$/, '').trim();
  ...
  if (cue.length > cap && deepUsed) {
    ...
    cue = 'با صدای ' + CFG.TTS_STYLE_BASE + (st2 ? '، ' + st2 : '') +
          '. ' + deep;
    cue = cue.replace(/\s+/g, ' ').trim();
    // ══ کفِ محافظ: نصفِ یک دستور، دستور نیست ══
    // حساب بالا دقیق است ولی حدِ پایین ندارد: اگر روزی خودِ یادآور بلندتر
    // از جایِ باقی‌مانده شود (کارت ۲۰۲۱ بایت است و فقط یک بندش برداشته
    // شده، پس بلندتر شدنش پیش‌بینی‌شده است)، بریدنِ ته آن را نصفه می‌کند و
    // چیزی به مدل می‌رسد که می‌تواند روی حرفِ نفی تمام شود. پس یا کامل، یا
    // اصلاً — و طبقِ قاعدهٔ ۵٫۹۶، وقتی جا به صفر می‌رسد دلیلش نوشته می‌شود.
    if (cue.length > cap || cue.indexOf(deep) === -1) {
      cue = ('با صدای ' + CFG.TTS_STYLE_BASE).replace(/\s+/g, ' ').trim();
      deepUsed = false;
      if (!_styleFitWarned) {
        _styleFitWarned = true;
        try {
          logLine_('یادآورِ سبک در سقفِ دستور جا نشد (' + deep.length +
                   ' نویسه، سقف ' + cap + ')؛ بی یادآور فرستاده شد.');
        } catch (eW) {}
      }
    }
  }
...
function ttsCueOffFinding_(model, why) {
  try {
    logSelfFinding_(null, {
      ...
      detail: 'مدل «' + model + '» به هر دو قالب (systemInstruction و ' +
              'interactions.instructions) پاسخِ ۴۰۰ داد: ' +
      ...
      instruction: 'خاموشیْ سقوطِ امن است (بی‌لحن، نه بی‌مرز) و نباید به ' +
                   ...
                   'صدای بلند است. از ۷٫۰۲ هر دو قالب (`generateContent` و ' +
                   '`interactions`) پیش از این حکم خودکار امتحان شده‌اند، پس ' +
                   'راهِ بازمانده یکی است: مدلِ گفتارسازِ دیگری که ' +
                   'systemInstruction/instructions را بپذیرد. تا وقتی چنین ' +
                   'مدلی نیامده، این ردیف را SKIPPED کن تا هر روز تکرار نشود.',
      owner: 'کد'
    });
  } catch (e) {}
}
        ...
        if (m.indexOf('HTTP 4') !== -1 && m.indexOf('429') === -1) {
          ...
          if (withCue !== false) {
            ...
            if (ttsCueRejected_(m)) {
              /* ══ مودِ دیگر هم دستور دارد — امتحانش کن، بعد تسلیم شو (۷٫۰۲) ══
                 `modes` از همان اول هر دو قالب را با یک `cue` ساخته
                 (`ttsPayloads_`): هم `generateContent.systemInstruction`
                 هم `interactions.instructions`. تا امروز اگر مودِ اول رد
                 می‌کرد، حکمِ «این مدل دستور نمی‌پذیرد» بی‌آنکه مودِ دوم
                 حتی یک‌بار امتحان شود صادر می‌شد — یعنی مسیرِ Interactions
 ...
 * ۶٫۷۸ فقط یک رسمِ نوشتاری را بخشید. سه هفتهٔ بعد `speakSkip` باز روی ۵۰٪
 * ایستاد و نمونه‌اش رسمِ *دیگرِ* همان کسرهٔ اضافه بود: «زمینه» ← «زمینه‌ی» —
 * نیم‌فاصله+ی، رسمِ رایج‌ترِ کسرهٔ اضافه پس از «ه»ی ساکن در نثرِ امروزی
 * («خانه‌ی من» = «خانهٔ من»). چون «ی» حرفِ معنادار است نه اعراب، این افزوده
 * را `speakGraft_` هم‌ترازیِ حرف‌به‌حرف می‌بیند و می‌شکند — دقیقاً همان
 * بیماری که ۶٫۷۸ برای رسمِ اول درمان کرد، این‌بار در لباسِ رسمِ دوم.
 *
 * بخشش اینجا هم یک‌طرفه و مستقل از رسمِ همزه است: متنِ خامی که خودش جایی
 * از «ه‌ی» استفاده کرده، آن مورد را دیگر نمی‌بخشد. `(?!ی)` عمداً هست —
 * «ه‌یی»/«ه‌یا» ادامهٔ واژه‌اند، نه کسرهٔ اضافه، و نباید بریده شوند.
 */
function speakEzafePrep_(plain, vowelled) {
  ...
  // آن‌ها اعرابند، نه بخشی از الگو، پس با [ً-ْ]* رد می‌شوند. و هر بار
  // regex‌ِ تازه: نمونهٔ globalِ مشترک با test()+replace() پیاپی lastIndex
  // را جا می‌گذارد و مسابقهٔ اولِ خودش را می‌پرد.
  var EZ_YE_RE = /(ه)[ً-ٕٓ-ٰٟ]*‌ی(?!ی)/;
  if (!EZ_YE_RE.test(p)) {
    v = v.replace(new RegExp(EZ_YE_RE.source, 'g'), '$1');
  }
  return { p: p, v: v };
}
...
var SPEAK_TRAPS = [
  ...
  /* ۷٫۶۹ — گزارشِ صاحبِ برنامه، با مثالِ خودش: «توجیه» را گوینده
     `tawjih` می‌خواند و «موضوع» را `maowzoo`. درستشان `tojih` و
     `mozu` است. این «واوِ عطف» نیست — واوِ **داخلِ** واژه است، و
     ریشه‌اش عربی‌خوانی است: دوگانه‌واکهٔ عربیِ «اَو» در فارسیِ معیار
     وجود ندارد و به /o/ (یا /u/) تبدیل شده. بندِ «لهجه» همین را کلی
     گفته بود («نه عربی‌خوانیِ واژه‌های عربی‌تبار») و این دقیق‌ترین و
     پرتکرارترین مصداقش است — کلی گفتن کافی نبود. */
  'واوِ داخلِ واژه در وام‌واژهٔ عربی‌تبار — پرتکرارترین خطای این گوینده. ' +
  'دوگانه‌واکهٔ «اَو» در فارسیِ معیار **نیست**: «توجیه» = تُوجیه (نه ' +
  '«تَوجیه» که `tawjih` شنیده می‌شود)، «موضوع» = مُوضوع (نه `maowzoo`). ' +
  'همین برای تُوضیح، تُولید، تُوحید، مُورِد، مُوقِع، مُوقِعیَّت، مُوجود. ' +
  '**ولی هر واوی این‌طور نیست:** جایی که پس از واو تشدید بیاید، واو ' +
  'همخوانِ /v/ است و حرفِ اول فتحه می‌گیرد — «تَوَجُّه»، «تَوَسُّط»، ' +
  '«تَوَقُّف»، «تَوَرُّم»، «مُوَفَّق». قاعده: تشدید داری؟ واو /v/ است. ' +
  'نداری؟ واو واکه است و حرفِ اول ضمه می‌گیرد. اشتباهِ این دو با هم، ' +
  'واژه را از آن‌که بود بدتر می‌کند، پس اول بشمار بعد اعراب بگذار.',
  ...
  /* ۶٫۵۵ — گزارشِ صاحبِ برنامه، با مثالِ خودش: «درآمدی معاصر» را گوینده
     «درآمدیِ معاصر» خواند — انگار «درآمد» یعنی پول و «ی» کسرهٔ اضافه
     گرفته. در حالی که آن‌جا «درآمد» یعنی مقدمه و «ی» نکره است و به صفتِ
     پسین بی‌کسره وصل می‌شود. اعرابِ درستِ حرف‌ها کافی نیست؛ باید اول
     معلوم باشد واژه در این جمله *کدام* واژه است. */
  '**اول معنا، بعد اعراب.** واژهٔ چندمعنا را تا معنایش در همین جمله روشن ' +
  'نشود اعراب نگذار: «درآمدی معاصر بر معرفت‌شناسی» — این «درآمد» یعنی ' +
  'مقدمه، و «ی» یِ نکره است؛ پس «دَرآمَدی مُعاصِر» بی‌کسرهٔ اضافه، نه ' +
  '«درآمدیِ معاصر» که یعنی پولی که معاصر است. همین برای «شیرِ آب/شیرِ ' +
  'جنگل»، «دورهٔ من گذشته/دُورِ من»، «بارِ علمی/بارِ کامیون». قاعده: معنای ' +
  'جمله را بفهم، بعد تصمیم بگیر این «ی» نکره است یا وحدت یا کسرهٔ اضافه، ' +
  'و این واژه کدام هم‌نگاشت است.',

  'یِ نکره/وحدت در برابرِ کسرهٔ اضافه. «کتابی خواندم» (نکره، بی‌کسره) در ' +
  'برابرِ «کتابِ خوبی». صفتِ پسین پس از یِ نکره کسره نمی‌گیرد: «درآمدی ' +
  'معاصر»، «نگاهی تازه»، «تجربه‌ای عجیب» — هر سه بی‌کسره وصل می‌شوند.',

  'لهجه: فارسیِ معیارِ تهرانِ امروز، یکدست در کلِ متن. نه عربی‌خوانیِ ' +
  'واژه‌های عربی‌تبار، نه کشیدنِ واکه‌ها به سبکِ محلی. اگر اعرابی که ' +
  'می‌گذاری واژه را از تلفظِ معیارِ تهرانی دور می‌کند، همان اعراب غلط است.'
];
...
function vowelizePiece_(piece) {
  ...
  var prompt =
    ...
    '**اول هر جمله را بفهم، بعد اعراب بگذار**: واژهٔ چندمعنا اعرابش را از ' +
    'معنایش در همین جمله می‌گیرد، نه از شکلِ آشناترش.\n' +
    'ابزارهایت:\n' +
...
function speakReviewPiece_(plain, vowelled) {
  var prompt =
    ...
    'و دو چیز را جمله‌به‌جمله بسنج، چون پرتکرارترین شکایتِ شنونده همین‌هاست:\n' +
    '- **کسرهٔ اضافهٔ نابه‌جا روی یِ نکره** («درآمدی معاصر» نباید «درآمدیِ ' +
    'معاصر» شود) و برعکس، کسرهٔ اضافهٔ جاافتاده؛\n' +
    '- **هم‌نگاشتی که با معنای جمله نمی‌خواند** — اول بفهم جمله چه می‌گوید، ' +
    'بعد ببین اعرابِ گذاشته‌شده همان واژه است یا واژهٔ دیگری با همین املا.\n\n' +
 ...
 */
/**
 * خانهٔ مُهر — **به ازای هر برنامه یکی** (۷٫۵۲).
 *
 * ══ چرا این تابع وجود دارد ══
 * ۷٫۴۴ این نگهبان را ساخت و هر دو برنامه را در **یک** ویژگی نوشت:
 * `'<show>|<date>'`. پس مُهرِ دومی مُهرِ اولی را پاک می‌کرد. ترتیبِ
 * واقعیِ تریگرها «از همه جا از همه رنگ» ۰۴:۰۰ و «درس‌نامه» ۰۵:۰۰ است،
 * یعنی تا ساعتِ هفت خانه `special|…` بود و هر مسیری که دوباره به
 * «قسمتِ تازه بساز» می‌رسید، از نگهبان رد می‌شد.
 *
 * ۲۴ سپتامبر همین شد: «از همه جا از همه رنگ» قسمتِ ۵۱ (۰۷:۱۵) و
 * قسمتِ ۵۲ (۰۷:۵۲) — **اولین صبحی که این نگهبان زنده بود** (۷٫۴۶ روزِ
 * پیش ساعتِ ۰۹:۴۳ نصب شد، یعنی پس از قسمت‌های آن روز).
 *
 * قاعده‌ای که این پرونده از ۵٫۹۶ دارد، یک نسخه آن‌طرف‌تر تکرار شد:
 * **زنگی که دو موضوع در آن شریک باشند، زنگِ هیچ‌کس نیست.** اینجا
 * مُهر بود نه زنگ، و شکل همان است. خانه‌ها حالا ساختاراً جدایند، نه
 * اینکه فقط با یک برچسب از هم تفکیک شوند.
 */
function epMadeKey_(show) {
  return String(PK.EP_MADE_DAY) + '_' + String(show || '');
}
...
function epMadeToday_(show) {
  try {
    ...
    var hit = function (v) {
      ...
      return p[0] === String(show) && p[1] === today;
    };
    if (hit(props_().getProperty(epMadeKey_(show)))) return true;
    /* ══ خانهٔ مشترکِ ۷٫۴۴ فقط **خوانده** می‌شود، و هرگز پاک نمی‌شود ══
       موتور می‌تواند وسطِ روز نصب شود — ۷٫۴۶ روزِ ۲۳ سپتامبر ساعتِ
       ۰۹:۴۳ نصب شد، یعنی پس از قسمت‌های آن صبح. بی این خط، برنامه‌ای
       که همان روز و با کدِ قبلی قسمتش را ساخته بود، خانهٔ تازه‌اش خالی
       می‌مانْد و یک قسمتِ اضافه می‌گرفت.
       و اینجاست که `p[0] === show` واقعاً کار می‌کند و صرفاً یک بندِ
       محکم‌کاری نیست: خانهٔ مشترک می‌تواند مالِ **آن یکی** برنامه باشد،
       و آن‌وقت «ساخته شده» جوابِ غلط است.
       پاکش هم نمی‌کنیم: پاک‌کردنش مُهرِ برنامهٔ دیگر را هم می‌بَرد و
       همان سوراخ از درِ دیگر باز می‌شود. خودش با تغییرِ تاریخ می‌میرد. */
    return hit(props_().getProperty(PK.EP_MADE_DAY));
  } catch (e) { return false; }   // نشد ≠ ساخته شده
}

function epMarkMade_(show) {
  try {
    props_().setProperty(epMadeKey_(show),
