...
function selfUpdateDaily() {
  ...
  /* و هرسِ ردیف‌های کهنهٔ خودِ تب (۶٫۸۶) — کنارِ بقیهٔ خانه‌تکانی، نه پشتِ
     نگهبانِ بودجه: یک بازخوانیِ تب است و اگر جا نیفتد، صف همان می‌مانَد که
     امروز هست، یعنی ۶۵ ردیفی که هیچ درِ خروجِ خودکاری ندارند. */
  try { reportStale_(getHub_()); } catch (eRS) { logLine_('هرسِ ردیف‌های کهنه نشد: ' + eRS.message); }
  ...
  // جدولِ «صداها»: ردیفِ بذر یک بار ساخته می‌شود و **خاموش** است. خبرش
  // یک بار می‌رود، وگرنه ردیفی در یک تبِ تازه را کسی پیدا نمی‌کند —
  // همان درسِ «چیزی که فقط در یک شیت زندگی کند، دیده نمی‌شود».
  try {
    if (personaSeed_()) {
      // ══ چرا اینجا نمی‌نویسد «روشنش کن» ══
      // نیمی از کار آماده است (شیوهٔ خواندن) و نیمِ دیگر نه (رنگِ صدا،
      // که بیرون از موتور اجرا می‌شود). صاحبِ برنامه صریح گفت:
      // «شیوهٔ خواندنِ رضوی روی صدای جمینای بی‌معنی است.» دعوت به
      // روشن کردنِ زودهنگام، همان «دستورِ کهنه»ای می‌شود که این
      // مخزن یک بخشِ کامل دربارهٔ آن دارد — با این تفاوت که از روزِ
      // اول کهنه است.
      mailQueue_('صدای تازه در جدولِ «صداها» — فعلاً خاموش بماند',
                 'ردیفِ «بهروز رضوی» در تبِ «صداها» ساخته شد و ' +
                 '**خاموش** است. همین درست است: این ردیف فقط شیوهٔ ' +
                 'خواندن را عوض می‌کند (مکث و کشش و دامنه)، نه رنگِ ' +
                 'صدا را. رنگِ صدا بیرون از موتور اجرا می‌شود و پلش ' +
                 'هنوز ساخته نشده. تا آن وقت روشنش نکنید؛ خبرش ' +
                 'داده می‌شود.');
    }
  } catch (ePS) { logLine_('بذرِ جدولِ صداها ناموفق: ' + ePS.message); }
  ...
  // یکی‌کردنِ نسخه‌های هم‌نامِ _MUSIC-FEED.json — بی‌قیدوشرط، حتی وقتی
  ...
  /* ══ گویندهٔ تازه (بخشِ ۳۳) — و چرا از بلوکِ «نخستین اجرا» بیرون آمد (۷٫۲۹) ══

     ۲۱ سپتامبر، نخستین استفادهٔ واقعیِ این بخش: صاحبِ برنامه ساعت ۲۲:۲۳
     پوشهٔ «محمد تقی پور گلدوز» را با ۲۷ فایلِ mp3 (~۹۷ مگابایت) گذاشت.
     کارِ شبانه ۰۲:۳۰ دوید و `_VOICE-QUEUE.json` **دست نخورد**. هیچ خطایی
     هم بلند نشد.

     علت ساختاری بود، نه کدِ بخشِ ۳۳: همان شب نسخهٔ ۷٫۲۷ نصب شد. اجرای
     **اول** با کدِ قدیمی (۷٫۱۶) دوید — کدی که اصلاً بخشِ ۳۳ نداشت — و
     `afterCodeSwap` اجرای دومی با کدِ تازه به راه انداخت. در آن اجرا
     `night.first` دیگر **false** بود، پس کلِ بلوکِ «کارهای ارزان» رد شد و
     `vintNightly_` هرگز صدا زده نشد. نتیجه: شبی که نسخهٔ تازه‌ای نصب
     می‌شود، هر کارِ تازه‌ای که داخلِ آن بلوک باشد **آن شب اصلاً اجرا
     نمی‌شود** — دقیقاً شبی که تازه آمده است.

     این همان شکلِ آشناست، یک لایه بالاتر: نگهبان درست بود، آستانه درست
     بود، و مسیری که باید موفق می‌شد تا اجرا اتفاق بیفتد، وجود نداشت.

     بیرون آوردنش امن است چون `vintNightly_` **idempotent** است: صف را با
     rev بالاتر دوباره می‌نویسد و هیچ چیزی را دو بار نمی‌سازد. و
     `_nightSkipTo` جلوی تکرارِ بی‌جا در همان شب را می‌گیرد. `srcNightly_`
     عمداً داخل ماند — آن نصب می‌کند، و نصبِ دوباره بی‌خطر نیست.

     و **پیش از** کارهای سنگین، چون پاسخِ اکشن اگر امشب خوانده نشود،
     فردا شب دو برابر عقب است. */
  /* ══ آیا گزارشِ روزانه رفت؟ (۷٫۶۳) ══
     بیرونِ بلوکِ «نخستین اجرا» به همان دلیلِ ۷٫۲۹، و **بی نگهبانِ زمان**:
     دو خواندنِ Properties و یک نگاه به `health` که همین حالا در دست است.
     اگر پشتِ `nightHas_` می‌نشست، شبی که وقت کم می‌آید هم شبی است که
     «هیچ گزارشی نرفته» باید گفته شود — و همان شب ساکت می‌ماند.
     و قرینه‌اش: `healthCheck` خبرِ مرگِ شبانه را می‌دهد (`nightDeath_`)،
     شبانه خبرِ مرگِ سلامت را. هیچ‌کدام شاهدِ خودش نیست. */
  ...
  /* پلِ رنگِ صدا — همان دلیلِ بالا: پاسخی که امشب برداشته نشود، فردا شب
     دو برابر عقب است. و ارزان است: خواندنِ یک نقشه و یک صف، بی هیچ
     پردازشِ صوتی (آن بیرون اتفاق می‌افتد). */
  if (nightHas_(25000, 'پلِ رنگِ صدا')) {
    try { vbrNightly_(null); }
    catch (eVB) { logLine_('دورِ پلِ صدا ناموفق: ' + eVB.message); }
  }
  ...
  /* ══ چرا این دو از دلِ بلوکِ جزوه بیرون آمدند (۶٫۹۰) ══
     تا امروز هر دو **داخلِ** همان `if` بودند، یعنی نگهبانِ زمانشان بعد از
     کاوشِ ۲۵ مجموعه و دو فراخوانِ مدل سنجیده می‌شد — و تقریباً هیچ‌وقت
     ۶۰ ثانیه نمانده بود. نتیجه: «نمودارهای جزوه: هنوز دوری اجرا نشده»،
     روزها، به‌عنوان یک جملهٔ خنثی در گزارش.
     بدتر: چون تودرتو بودند، شبی که بلوکِ بیرونی هم جا نمی‌شد، نگهبانشان
     اصلاً **سنجیده نمی‌شد** — پس نه اجرا می‌شدند، نه گرسنگی‌شان شمرده
     می‌شد. حالا هم‌ترازِ بقیه‌اند و شمارندهٔ گرسنگیِ ۶٫۸۹ می‌بیندشان. */
  ...
  if (CFG.HANDOUT_ENABLED !== false && nightHas_(60000, 'نمودارهای جزوه')) {
    try { handoutVizSweep_(Number(CFG.HANDOUT_VIZ_SWEEP) || 6, 120000); }
    catch (eVz) { logLine_('جاروی نمودارهای جزوه ناموفق: ' + eVz.message); }
  }
  if (CFG.HANDOUT_ENABLED !== false && nightHas_(30000, 'مرتب‌سازیِ عنوانِ فصل‌های جزوه')) {
    try {
  ...
  /* ══ اثر انگشتِ معنایی (بخشِ ۳۵) ══
     پشتِ `nightHas_` مثلِ هر چیزِ دیگری که به شبانه اضافه می‌شود — قاعدهٔ
     ۵٫۶۸ استثنا ندارد. و **پس از** خانه‌داری، چون یک شب دیر ساخته شدنِ
     بردارها هزینه‌ای ندارد ولی یک شب نصب‌نشدنِ کد دارد.

     بودجه‌اش از همه بیشتر است چون کارِ واقعی می‌کند: تا ۱۲۰۰ ردیف، هر
     دسته یک فراخوانِ شبکه. مکان‌نما دارد، پس شبی که جا نشود فردا از
     همان‌جا ادامه می‌دهد — و شمارندهٔ گرسنگیِ ۶٫۸۹ می‌بیندش اگر هرگز
     نوبتش نرسد. */
  /* نگهبانش بزرگ است، عمداً — ولی نه بزرگ‌تر از کلِ بودجه (۷٫۳۰).
     ۷٫۲۸ این عدد را به ۳۰۰۰۰۰ رساند تا «در نخستین اجرای هر شب تقریباً
     هیچ‌وقت رد نشود»، بی‌آنکه بسنجد کلِ `NIGHT_BUDGET_MS` خودش فقط
     ۲۷۰۰۰۰ است. یعنی از همان شبِ نصب، `nightLeft_()` هرگز نمی‌توانست به
     ۳۰۰۰۰۰ برسد — نه امشب، نه هیچ شبِ دیگر — و `nightHas_` همیشه false
     برمی‌گرداند. چون این گیت اولین‌بار `_nightMore` را می‌نشاند، هر
     بلوکِ بعدی (از جمله `گذشته‌نگرِ نسبتِ ارجاع‌ها`ی همین شب) هم بی‌صدا
     رد می‌شد — نه فقط اثر انگشت. ۲۳۰۰۰۰ همان عددی است که در ۷٫۲۷ واقعاً
     کار می‌کرد و هنوز حاشیهٔ کافی روی بودجهٔ کارِ درونی (`EMB_BUDGET_MS`
     ۲۰۰۰۰۰) دارد. `run_oneshot_test.js` ۳۳ اکنون هر عددِ `nightHas_` را
     از رویِ خودِ کد استخراج می‌کند و مطمئن می‌شود هیچ‌کدام از
     `NIGHT_BUDGET_MS` بزرگ‌تر نیست — تا نگهبانِ بعدی هم همین اشتباه را
     نسازد. */
  if (CFG.EMB_ON !== false && nightHas_(230000, 'اثر انگشتِ معنایی')) {
    try { embNightly_(); }
    catch (eEm) { logLine_('دورِ اثر انگشتِ معنایی ناموفق: ' + eEm.message); }
  }
  ...
  /* ── نمونهٔ «روحِ خواندن» — وقتی یادآور عوض شود، نه هر شب ──
   * صاحبِ برنامه هیچ شیتی باز نمی‌کند و هیچ منویی نمی‌زند، پس نمونه‌ای که
   * فقط با دکمه ساخته شود، ساخته نمی‌شود. ولی هر شب ساختنش هم دو فراخوانِ
   * مدل و دو نوشتنِ درایو است برای فایلی که عوض نمی‌شود.
   *
   * ══ و نشانه چرا شمارهٔ نسخه نیست ══
   * اولین نگارشِ این بلوک `CODE_VERSION` را نشانه می‌گذاشت، با این استدلال
   * که «اگر یادآور عوض شود نمونهٔ تازه لازم است». ولی این مخزن روزی یک
   * نسخه می‌دهد (۷٫۰۵ تا ۷٫۰۹ یعنی پنج روزِ پشتِ‌هم)، پس آن نشانه **هر شب**
   * عوض می‌شد و نمونه هر شب دوباره ساخته می‌شد: دقیقاً همان کاری که کامنت
   * می‌گفت نمی‌کند. نشانه حالا خودِ متنِ یادآور است — همان چیزی که از اول
   * ادعا می‌شد. اگر یادآور عوض شود نمونهٔ تازه می‌آید، و اگر نه، هیچ.
   *
   * بودجه ۴۰ ثانیه نیست: هر نمونه از `ttsChunkTry_` می‌گذرد که تا سه تلاش
  ...
  try {
    ...
    if (spKey && spDone !== spKey && nightHas_(150000, 'نمونهٔ روحِ خواندن')) {
      var spR = runStyleProbe();
      if (spR && spR.ok) props_().setProperty(PK.STYLE_PROBE_DONE, spKey);
    }
  } catch (eSP) { logLine_('نمونهٔ سبک: ' + eSP.message); }
...
function markCodeRowsInstalled_(version) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    /* ══ و کلیدِ یافته هم پذیرفته است، نه فقط شناسهٔ کاملِ ردیف (۷٫۴۸) ══
       شناسهٔ ردیف `ENG-<yyyyMMdd-HHmm>#<کلید>` است، و آن مُهرِ زمانی زمانِ
       **نخستین** دیده‌شدنِ یافته است — نه امروز. کسی که امروز همان یافته را
       حل می‌کند آن تاریخ را ندارد، و برای به‌دست آوردنش باید هابِ ۲۹
       مگابایتی خوانده شود.
       ۷٫۴۲ نوشت «دری که آدم باید بازش کند، در نیست» و تنها درِ بستنِ ردیف
       را همین‌جا گذاشت. این یک لایه پایین‌تر است و همان نتیجه را می‌دهد:
       **دری که کلیدش به‌سختی به دست بیاید هم در نیست.** از سی نسخه یکی
       این فهرست را پر کرده بود؛ علتش تنبلی نبود، نداشتنِ شناسه بود.
       پس `sourceReportIds: ["tts-cue-unsupported"]` هم می‌بندد.

       و «#۵» کلید نیست: در شناسهٔ گزارش‌های معمولی (`RPT-…#5`) آن عدد
       شمارهٔ یافته در همان گزارش است. پذیرفتنش یعنی نوشتنِ «5» در بیانیه
       ردیفی را ببندد که هیچ ربطی به این نسخه ندارد. */
    var rid = String(r[RC.ID - 1]);
    var rkey = '';
    var hashAt = rid.lastIndexOf('#');
    if (hashAt !== -1) {
      var kk = rid.slice(hashAt + 1);
      if (kk && !/^\d+$/.test(kk)) rkey = kk;
    }
