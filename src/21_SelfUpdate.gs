...
function selfUpdateDaily() {
  ...
  try {
    if (personaSeed_()) {
      mailQueue_('صدای تازه در جدولِ «صداها»',
                 'ردیفِ «بهروز رضوی» در تبِ «صداها» ساخته شد و فعلاً ' +
                 '**خاموش** است. ستونِ «فعال» را «بله» کنید تا هر سومین ' +
                 'قسمت با شیوهٔ خواندنِ او اجرا شود. این فقط سبکِ اجراست ' +
                 '(مکث و کشش و دامنه)، نه رنگِ صدا.');
    }
