...
function auditModel_(snap) {
  ...
  /* درس‌نامه «آموزش» است و برنامهٔ ترکیبی «گزارش». یک معیار برای هر دو،
     یکی از آن دو را هر شب بی‌دلیل رد می‌کند. */
  var isLesson = String(snap.show) === ENRICH_SHOW_SPECIAL;
  var prompt = [
    ...
    /* ══ چرا درس‌نامه معیارِ خودش را دارد (۶٫۱۰) ══
     * معیارِ زیر برای «از همه جا از همه رنگ» نوشته شده بود، جایی که موادِ
     * خام تکه‌های جدا از هم‌اند (یک کلیپ، یک عکس، یک سند) و کارِ متن
     * **گزارش‌کردنِ** آن‌هاست. درس‌نامه دقیقاً برعکس است: ماده یک متنِ
     * پیوستهٔ درسی است و کارِ قسمت **آموزش‌دادنِ** آن — توضیح، مثال،
     * بازگفتن به زبانِ ساده. با معیارِ برنامهٔ ترکیبی، هر جملهٔ آموزشی
     * «فراتر از خام» و هر جملهٔ ربط «پیوندِ ساختگی» شمرده می‌شود.
     *
     * و این فقط نویز نبود: یافته هر شب یک دستور به قسمتِ بعد می‌داد که
     * «چیزی به خام اضافه نکن» — یعنی بازخوردی که درس را **بدتر** می‌کرد.
     * قسمت ۱۵ هر شش بخشش هم «فراتر» بود هم «ساختگی»؛ صد درصدِ یک قسمت
     * غلط‌بودن، تقریباً همیشه یعنی معیار غلط است، نه متن. */
    '۱) fit — آیا این متنِ درسی برای این بخش درست انتخاب شده؟',
    '   «مناسب» یعنی بخش واقعاً همین مطلبِ درسی را می‌آموزد.',
    '   «نامناسب» یعنی ماده ربطی به موضوعِ بخش ندارد. اگر مطمئن نیستی «نامعلوم».',
    '',
    '۲) linked — این‌جا مادهٔ خام **یک متنِ پیوستهٔ درسی** است، نه چند تکهٔ',
    '   جدا. پس پرسش این نیست که چطور به هم وصل شده‌اند. پرسش این است که',
    '   آیا ترتیبِ آموزش با خودِ درس می‌خوانَد یا مطلبی جابه‌جا و بی‌مقدمه',
    '   آمده. جمله‌های ربطِ آموزشی («حالا ببینیم…», «پس تا اینجا…») **ایراد',
    '   نیستند** — کارِ معلم‌اند. «واقعی» یا «ساختگی» یا «بی‌ربط».',
    '',
    '۳) faithful — کارِ این قسمت **آموزشِ** متنِ درسی است، نه نقلِ آن.',
    '   پس توضیح‌دادن، مثال‌زدن، ساده‌کردن، تکرار برای جاافتادن و مقدمه‌چینی',
    '   **وفادارند** — حتی اگر عینِ واژه‌هایشان در خام نباشد.',
    '   «فراتر» فقط وقتی است که ادعایی افزوده شده که در درس **ریشه ندارد**:',
