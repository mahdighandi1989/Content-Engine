...
function watchdogHeartbeats_(st) {
  ...
  out.push({ key: 'enrich', name: 'تسکِ غنی‌سازی',
             ...
             evidence: eEv ? (eEv.name || '—') + ' از ' + eEv.seen + ' پاسخ در ریشه' +
             ...
             fix: (taskIdle
               ...
               (eEv ? ' (تازه‌ترین پاسخی که دیده شد: ' + (eEv.name || 'هیچ') +
                      ' — از ' + eEv.seen + ' فایل' +
                      (eEv.error ? '؛ پیمایش افتاد: ' + eEv.error : '') + '.)' : '') });
...
function whNewestEnrich_(detail) {
  ...
  var at = newest ? Utilities.formatDate(new Date(newest), CFG.TIMEZONE, 'yyyy-MM-dd HH:mm') : '';
