...
function recToRow_(r) {
  return [r.fileId, kindLabel_(r.kind), r.date, r.sub || '',
          ...
          '', '',
          /* و سه ستونِ اثر انگشت (۷٫۲۶) — عمداً خالی. بخشِ ۳۵ پُرشان
             می‌کند، و بخشِ ۲ حق ندارد به بخشِ ۳۵ وابسته شود: وابستگیِ
...
function syncCatalog() {
  ...
  try {
    ...
    if (more) {
      ...
      // یکی‌کردنِ نسخه‌های هم‌نامِ _MUSIC-FEED.json — علاوه بر اجرای شبانه.
      // تسکِ غنی‌سازی هر ساعت یک فایلِ تازه می‌سازد (ابزارش نوشتنِ درجا
      // ندارد)، پس یک جاروی شبانه از پسش برنمی‌آید: هر شب یکی می‌شود و
      // ساعت‌ها بعد دوباره دو تا می‌شود، درست جایی که وارسیِ روز آن را
      // می‌بیند. syncCatalog هر ۲ ساعت می‌دود — فاصله را از ~۲۴ ساعت به
      // ~۲ ساعت می‌آورد.
      try { musicFeedDedup_(); } catch (eFD2) {}
