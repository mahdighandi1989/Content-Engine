...
function reportRow_(rep, f, idx, fp) {
  ...
  /* ══ شناسه باید یکتا باشد، وگرنه بستنِ یک ردیف ردیفِ دیگری را هم می‌بندد ══
   * تا امروز گزارشِ بی‌شناسه به رشتهٔ برهنهٔ 'RPT' برمی‌گشت، پس هر گزارشِ
   * بی‌شناسه ردیف‌های `RPT#1`، `RPT#2`… می‌ساخت — و این نام‌ها بینِ گزارش‌های
   * کاملاً متفاوت مشترک می‌شدند.
   *
   * هزینه‌اش را سه گزارشِ پیاپیِ ناظر نوشتند (۱۷، ۱۸ و ۱۹ سپتامبر): «هنوز
   * باز، هنوز جدی»، و هر بار ناظر از بستنِ ردیف‌های واقعاً پاسخ‌گرفته صرفِ‌نظر
   * کرد چون نمی‌توانست مطمئن باشد کدام ردیف را می‌بندد. یعنی یک خطِ کد،
   * سه روز، خودِ سازوکارِ خوداصلاحی را قفل کرده بود.
   *
   * ردیف‌های قدیمی دست نمی‌خورند — قاعدهٔ ثابتِ این مخزن: تحلیل‌های پیشین
   * هرگز خراب نمی‌شوند. این فقط شناسه‌های **تازه** را یکتا می‌کند. */
  var rid = String(rep.reportId || '').trim();
  if (!rid) {
    var stamp = String(rep.at || nowStr_()).replace(/[^0-9]/g, '').slice(0, 12);
    rid = 'RPT-' + (stamp || String(new Date().getTime()));
  }
  var id = rid + '#' + (idx + 1);
