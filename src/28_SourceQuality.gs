...
function sqRun_(budgetMs, force) {
  ...
  sqBaseSave_(base);
  /* ══ ۷٫۰۹: دوری که چیزی نسنجید، صفِ هفتگی را نباید مصرف کند ══
   * تا این نسخه `PK.SQ_AT` بی‌قید‌وشرط همین‌جا به‌روز می‌شد — حتی وقتی
   * `srcScriptGet_` برای **هر دو** تحلیلگر شکست خورده بود و `out.ran` صفر
   * ماند. یعنی یک شکستِ گذرا (شبکه، سهمیهٔ Apps Script API) هفت روزِ
   * کامل به تعویق می‌افتاد، بی هیچ ردی جز سکوت — دقیقاً چیزی که در دادهٔ
   * واقعی دیده شد: تبِ «کیفیتِ استخراج» از ۹ تا ۱۸ سپتامبر (بیش از یک
   * دورِ کامل) هیچ ردیفِ تازه‌ای نگرفت، در حالی که هر دو تحلیلگر از ۱۰
   * سپتامبر روی نسخهٔ اصلاح‌شده (مدلِ تازه) بودند و `sqStatus_` همچنان
   * حکمِ «مرده»ی پیش از اصلاح را نشان می‌داد. حالا نوبتِ هفتگی فقط وقتی
   * مصرف می‌شود که دستِ‌کم یک تحلیلگر واقعاً سنجیده شده باشد؛ دوری که
   * هیچ‌چیز نسنجید، فردا شب دوباره امتحان می‌کند، نه هفتهٔ بعد. */
  if (out.ran > 0) {
    try { props_().setProperty(PK.SQ_AT, nowStr_()); } catch (e) {}
  } else {
    logLine_('نظارتِ کیفیِ استخراج این دور چیزی نسنجید (کدِ زندهٔ هیچ ' +
             'تحلیلگری خوانده نشد) — نوبتِ هفتگی مصرف نشد، فردا دوباره.');
  }
  return out;
}
