 ...
 */
// ══ متنِ نمونهٔ سبک ══
// عمداً روایی و چندجمله‌ای با یک جملهٔ بلندِ قابلِ شکستن: اگر یادآور اثر
// کند، همین‌جاست که شنیده می‌شود — مکثِ درونِ جمله جایی معنا دارد که جمله
// جا برای مکث داشته باشد. یک جملهٔ کوتاه هیچ‌چیز را ثابت نمی‌کند.
//
// ══ و چرا اعراب‌دار است — درسی که ۷٫۱۰ گران داد ══
// نگارشِ اول این متن بی‌اعراب بود و کلِ قابلیت را بی‌صدا کُشت: `ttsCue_`
// برای متنِ بی‌اعراب عمداً یادآورِ **تلفظ** را انتخاب می‌کند و یادآورِ سبک
// اصلاً نمی‌نشیند. یعنی هر دو نمونه بایت‌به‌بایت یکی می‌شدند، تابع `✅ هر دو
// نمونه ساخته شد` می‌گفت، و صاحبِ برنامه با شنیدنِ دو فایلِ یکسان نتیجه
// می‌گرفت «کارت اثری ندارد» — یعنی گزارشی که فقط بی‌اطلاع نبود، گمراه‌کننده
// بود. متنِ نمونه باید از همان دری برود که متنِ واقعیِ قسمت می‌رود، و متنِ
// واقعی بعد از فازِ `speak` اعراب‌دار است.
var STYLE_PROBE_LINE =
  'شَبی از شَب‌هایِ پاییز بود، از آن شَب‌هایی که باد پُشتِ پَنجِره ' +
  'می‌ایستَد و نِمی‌رَوَد. مَردی که سال‌ها از آن خانه دور مانده بود، ' +
  'کِلید را در قُفل چَرخانْد و ایستاد. هیچ‌چیز سَرِ جایَش نَبود، و ' +
  'هَمه‌چیز هَمان‌جا بود.';
 ...
 * بی آن، هر تفاوتی که شنیده شود به کارت نسبت داده می‌شود — حتی اگر کارت
 * هیچ نکرده باشد. صاحبِ برنامه خودش این را خواست: «نمونه‌هاش رو این بار
 * با رنگ و روح بشنوم نه رنگ خالی». دو فایل، یک متن، یک صدا، یک لحن —
 * تنها متغیر همان یادآورِ سنجیده است.
 *
 * ══ چرا نتیجه‌اش به تنهایی «رنگ» ندارد ══
 * این‌جا فقط خوانشِ جمینای ساخته می‌شود. رنگِ رضوی از RVC می‌آید که در
 * Actions اجرا می‌شود، نه این‌جا. پس این دو فایل ورودیِ آن مرحله‌اند:
 * اول روح را بشنو، بعد همان‌ها رنگ می‌گیرند.
 */
function runStyleProbe() {
  var ui = ui_();
  var made = [], err = '';
  var folder = null;
  // ══ قفل، چون این تابع رفتارِ تولیدِ زنده را عوض می‌کند ══
  // قرینه‌اش `runVoiceAudition` قفل دارد، و آن فقط فایل می‌سازد. این یکی
  // پرچمی می‌گذارد که `ttsCue_` در هر تکه می‌خوانَد — اگر ساعتِ ۷:۰۳ دکمه
  ...
  if (!got) {
    ...
    if (ui) ui.alert('نمونهٔ روحِ خواندن', busy, ui.ButtonSet.OK); else console.log(busy);
    return { ok: false, made: [], error: 'busy', folder: '' };
  }
  try {
    var root = outFolder_();
    // در ریشهٔ OUTPUT نمی‌نویسیم: `outLayoutCheck_` هر نامِ ناشناخته را
    // «سرگردان» گزارش می‌کند و یک هشدارِ هرشبه برای کاری که درست است،
    // همان هشداری است که مردم یاد می‌گیرند نادیده بگیرند.
    var nm = CFG.VOICE_AUDIT_FOLDER || 'آزمونِ صدای گویندگان';
    ...
    /* ══ سنجه‌ای که خودش را نسنجد، تئاتر است ══
     *
     * ۷٫۱۰ همین‌جا شکست و درسش گران بود: متنِ نمونه بی‌اعراب بود، پس
     * `ttsCue_` شاخهٔ یادآورِ **تلفظ** را می‌گرفت و یادآورِ سبک اصلاً
     * نمی‌نشست. دو فایل بایت‌به‌بایت یکی می‌شدند، تابع «✅ هر دو نمونه ساخته
     * شد» می‌گفت، نشانهٔ «انجام شد» ثبت می‌شد و دیگر هرگز تکرار نمی‌شد — و
     * صاحبِ برنامه با شنیدنِ دو فایلِ یکسان نتیجه می‌گرفت «کارت اثری ندارد».
     * گزارشی که فقط بی‌اطلاع نباشد بلکه **گمراه‌کننده** باشد، از سکوت بدتر
     * است؛ همان درسی که «گزارشی که خوانده نشود» در این پرونده دارد.
     *
     * پس پیش از هر فایلی، همان چیزی سنجیده می‌شود که واقعاً فرستاده خواهد
     * شد — `ttsPayloads_`، نه `ttsCue_` — چون دستور می‌تواند جای دیگری هم
     * خاموش شود (`PK.TTS_CUE_OFF` برای مدلی که قالب را رد کرده). اگر دو
     * دستور یکی باشند، نمونه ساخته نمی‌شود و دلیلش گفته می‌شود.
     */
    var cueOfProbe_ = function (on) {
      styleProbeSet_(on);
      try {
        var p = ttsPayloads_(line, null, st, CFG.TTS_VOICE, true);
    ...
    if (cueOn === cueOff) {
      err = 'دستورِ هر دو نمونه یکی درآمد، پس دو فایلِ یکسان ساخته می‌شد و ' +
            'هیچ داوری‌ای ممکن نبود؛ چیزی نوشته نشد. دستور: ' +
            String(cueOn).slice(0, 120);
      plan = [];
    }
    for (var i = 0; i < plan.length; i++) {
      // ══ برداشتنِ پرچم شرطِ درستیِ کار است، نه نظافت ══
      // اگر اجرا وسطِ کار بیفتد و این خاصیت بمانَد، موتور تا ابد در حالتی
      // می‌مانْد که هیچ‌کس انتخابش نکرده — و چون `SPEAK_STYLE_ON` هنوز
  ...
  // حقیقتش جلو زده — و خواننده یا کارِ بی‌خود می‌کند یا یاد می‌گیرد پیام را
  // نخوانَد. و جوابِ `mailQueue_` **چک می‌شود**: صفِ خرابْ یعنی این خبر به
  // هیچ‌کس نرسیده، و خبری که نرسیده باید دستِ‌کم در سیاهه بماند.
  if (!mailQueue_('سبک', okAll ? 'نمونهٔ روحِ خواندن آماده شد'
                               : 'نمونهٔ روحِ خواندن ناقص ماند', m)) {
    logLine_('نمونهٔ سبک: صفِ ایمیل نپذیرفت — خبر نرفت.');
  }
  if (ui) ui.alert('نمونهٔ روحِ خواندن', m, ui.ButtonSet.OK); else console.log(m);
  return { ok: okAll, made: made, error: err,
           folder: folder ? folder.getUrl() : '' };
}
