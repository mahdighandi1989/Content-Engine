> نسخه: 5 — 2026-08-23
> برای نسخهٔ موتور: 5.56
> این فایل «بدنهٔ» دستورِ تسکِ غنی‌سازی است. تسک آن را از درایو می‌خواند.
> به‌روزرسانی = ساختنِ فایلِ تازه با شمارهٔ بالاتر (`_PROMPT-enrich-v6.md`).
> هرگز این فایل را بازنویسی نکنید؛ تاریخچه باید بماند.
>
> تغییرِ نسخهٔ ۵ نسبت به ۴ — باز هم فقط بخشِ ۸:
> نسخهٔ ۴ گفت «نشانیِ WAV بنویس» ولی نگفت **کجا**. بیشترِ سایت‌های موسیقیِ
> آزاد فقط MP3 می‌دهند و Apps Script رمزگشای MP3 ندارد، پس محتمل‌ترین نتیجه
> «رد — WAV نیست» بود، هر شب. حالا فهرستِ منابعی که واقعاً WAV می‌دهند اینجا
> هست، با دستورالعملی که **پیش از پیشنهاد** مطمئن شوی.
> ضمناً از ۵٫۵۶ خودِ موتور هم شب‌ها archive.org را می‌گردد و نامزد به فهرست
> اضافه می‌کند. کارِ تو جایگزینش نیست، مکمّلش است: تو حال‌وهوا و مناسبت را
> می‌فهمی، جست‌وجوی خودکار نمی‌فهمد.
...
3. **Where to look — sources that actually serve WAV.** This is the whole
   ...
   **Do not propose:** freesound.org (downloads need an API key and previews are
   MP3/OGG), the BBC sound-effects archive (its licence does not cover a
   published podcast), or any URL you found but did not verify.
...
8. **The engine searches too — you are not alone in this, and not redundant.**
   ...
   `"by": "موتور — گشتنِ خودکار"`. Leave those alone. What the automatic search
