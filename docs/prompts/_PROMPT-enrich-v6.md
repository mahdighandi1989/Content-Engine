> نسخه: 6 — 2026-08-23
> برای نسخهٔ موتور: 5.65
> این فایل «بدنهٔ» دستورِ تسکِ غنی‌سازی است. تسک آن را از درایو می‌خواند.
> به‌روزرسانی = ساختنِ فایلِ تازه با شمارهٔ بالاتر (`_PROMPT-enrich-v7.md`).
> هرگز این فایل را بازنویسی نکنید؛ تاریخچه باید بماند.
>
> تغییرِ نسخهٔ ۶ نسبت به ۵ — باز هم فقط بخشِ ۸، ولی این بار یک خرابیِ واقعی:
> ۲۳ اوت ساعت ۱۵، در ریشهٔ OUTPUT **سه تا `_MUSIC-FEED.json`** بود (۱۴:۰۴،
> ۱۴:۲۷، ۱۵:۲۷) — هر اجرا یکی تازه ساخته بود به‌جای به‌روزکردنِ همان.
> `getFilesByName` فقط یکی را برمی‌گرداند و کدام‌یک تضمینی نیست، پس موتور
> می‌توانست نسخهٔ کهنه را بخواند؛ و وقتی می‌نوشت، بقیه را به سطلِ زباله
> می‌بُرد. یعنی سه نامزدی که همان اجرا پیدا کرده بودی، خوانده‌نشده پاک
> می‌شدند. موتور از ۵٫۶۵ تازه‌ترین را می‌خواند و همه را با هم یکی می‌کند و
> تکرار را هشدار می‌دهد — ولی راهِ درست این است که اصلاً تکراری ساخته نشود.
> ضمناً: pixabay اسمش صریح آمد (MP3 است، بی‌فایده)، opengameart اضافه شد
> (امتحان‌شده — سه فایلِ اولِ بانک از همان‌جا آمد)، و از ۵٫۶۴ موتور دنبالِ
> **جلوهٔ صوتی** هم می‌گردد، پس افکت هم پیشنهاد بده.
...
3. **Where to look — sources that actually serve WAV.** This is the whole
   ...
   **Do not propose:** **pixabay.com** — it has roughly 130 000 sound effects and
   they are all MP3; the engine has no MP3 decoder and from 5.65 rejects such a
   URL on sight, without downloading. The same goes for Free Music Archive,
   Incompetech, Chosic and Bensound. Also do not propose freesound.org (downloads
...
9. If you *can* upload a real WAV directly into «موسیقی و افکت» (with its
