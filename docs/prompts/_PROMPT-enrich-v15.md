> نسخه: 15 — 2026-08-26
> برای نسخهٔ موتور: 6.20
> این فایل «بدنهٔ» دستورِ تسکِ غنی‌سازی است. تسک آن را از درایو می‌خواند.
> به‌روزرسانی = ساختنِ فایلِ تازه با شمارهٔ بالاتر (`_PROMPT-enrich-v16.md`).
...
>
> ══ تغییرِ نسخهٔ ۱۳: کارِ تازه‌ات — ساختِ ویدئوی یوتیوب ══
>
> از ۵٫۹۷ پادکست‌ها در یوتیوب منتشر می‌شوند. یوتیوب فایلِ صوتی نمی‌پذیرد و
> **Apps Script نمی‌تواند ویدئو بسازد** — نه ffmpeg دارد، نه کتابخانه‌ای، و
> مهلتِ شش‌دقیقه‌ای هم اجازه نمی‌دهد. این تنها حلقهٔ گمشدهٔ زنجیره است، و
> کارِ توست. بقیه‌اش (کپشن، کاور، پلی‌لیست، انتشار) با موتور است.
>
> **در هر اجرا، پس از کارِ غنی‌سازی:** فایلِ `_YT-RENDER.json` را در ریشهٔ
> OUTPUT بخوان. هر ردیفی که `status` آن «در انتظار» است یعنی یک ویدئو
> می‌خواهد. برای هرکدام (حداکثر دو تا در هر اجرا، تا زمان کم نیاید):
>
> ۱. **آرایهٔ `audio` را به همان ترتیب** از درایو بگیر. **مهم:** بعضی
>    قسمت‌ها دو فایل‌اند («… یکجا ۱ از ۲» و «… یکجا ۲ از ۲») چون یک WAVِ
>    واحد از سقفِ درایو می‌گذشت. این‌ها **یک قسمتِ واحدند** و باید پشتِ‌هم
>    چسبانده شوند تا **یک** ویدئو بدهند. اگر فقط یکی‌شان را برداری، نیمی
>    از درس منتشر می‌شود و هیچ خطایی هم نمی‌دهد.
>
>    چسباندن با ffmpeg، بی رمزگشاییِ دوباره (همه ۲۴ کیلوهرتز، ۱۶ بیت،
>    تک‌کاناله‌اند، پس `-c copy` امن است):
>
>    ```
>    printf "file '%s'\n" part1.wav part2.wav > list.txt
>    ffmpeg -f concat -safe 0 -i list.txt -c copy joined.wav
>    ```
> ۲. `coverFileId` را بگیر (PNGِ ۱۶:۹ که موتور ساخته). اگر خالی بود، یک
>    زمینهٔ سادهٔ تک‌رنگِ ۱۲۸۰×۷۲۰ بساز.
> ۳. ffmpeg را از pypi بیاور — `pip install imageio-ffmpeg` — و مسیرِ
>    باینری‌اش را با `imageio_ffmpeg.get_ffmpeg_exe()` بگیر. **هیچ دسترسیِ
>    اینترنتیِ دیگری لازم نیست**؛ خودت گزارش داده‌ای که pypi در دسترس است.
> ۴. یک MP4 با تصویرِ ثابت بساز:
>
> ```
> ffmpeg -loop 1 -framerate 1 -i cover.png -i joined.wav \
>   -c:v libx264 -tune stillimage -pix_fmt yuv420p -r 1 \
>   -vf "scale=1280:720:force_original_aspect_ratio=decrease,\
>        pad=1280:720:(ow-iw)/2:(oh-ih)/2" \
>   -c:a aac -b:a 128k -ac 1 -shortest -movflags +faststart out.mp4
> ```
>
> ۵. فایل را با **همان `outName`** در **همان `folderId`** (پوشهٔ قسمت)
>    بگذار. موتور خودش پیدایش می‌کند، آپلود می‌کند و ردیف را می‌بندد.
>
> **سقفِ حجم: ۴۵ مگابایت.** آپلودِ Apps Script از ۵۰ مگابایت بالاتر نمی‌رود.
> با تنظیماتِ بالا یک قسمتِ چهارده‌دقیقه‌ای حدودِ ۱۵ مگابایت می‌شود؛ اگر
> بزرگ‌تر درآمد `-b:a 96k` را امتحان کن.
>
> **هیچ‌جای دیگری را دست نزن.** نه `_YT-RENDER.json` را ویرایش کن (موتور
> خودش می‌بندد)، نه فایلِ صوتی را، نه پوشهٔ دیگری را. فایلِ چسبانده‌شده
> موقتی است — در درایو نگذارش، فقط MP4 را بگذار.
>
> **پیش از رفتن، یک وارسی:** مدتِ MP4 را با `ffprobe` بگیر و با مجموعِ مدتِ
> فایل‌های ورودی بسنج. اگر کمتر بود یعنی بخشی جا افتاده — MP4 را **نگذار** و
> به‌جایش یافته‌ای با `key: "yt-render-short"` بنویس و بگو چه دیدی. ویدئوی
> نصفه‌ای که منتشر شود، برخلافِ انتشارِ عقب‌افتاده، برگشت‌پذیر نیست.
>
> **اگر نتوانستی** — pip کار نکرد، ffmpeg اجرا نشد، حجم بالا ماند — یک
> یافته در گزارشت بنویس با `key: "yt-render-failed"` و بگو دقیقاً کجا
> ایستاد و پیامِ خطا چه بود. **چیزی را بی‌صدا رها نکن**: تا وقتی این
> ویدئوها ساخته نشوند، هیچ قسمتی در یوتیوب منتشر نمی‌شود، و از بیرون
> شبیهِ «یوتیوب خاموش است» به‌نظر می‌رسد. همان چیزی که بانکِ موسیقی را
...
1. Read `_MUSIC-WISH.json`. **It now has two kinds of row, and the second one is
   ...
   `times` first. Use `en` as your search text, and §9 to convert and upload —
   pixabay is the right source for exactly this.
...
6b. **Sound effects — and now you are told exactly which ones.** From 5.64 the
   ...
   **anchor phrase** in the narration rather than at the top of a section — so an
   ...
   needed. **Answering those beats anything you would guess** — and you can reach
   sources the engine cannot, because you can convert MP3 (§9).
...
9. **The second path: convert it yourself and upload the WAV.** This is new in
   ...
   **ABSOLUTE: never ask the user to place a file.** Not in a report, not in a
   ...
   A missing track costs nothing. Handing the owner a chore costs the point of
   the whole system.
